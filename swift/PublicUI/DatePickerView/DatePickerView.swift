...
class DatePickerView: UIView, UIGestureRecognizerDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    /// 确定按钮背景色，默认蓝色
    public var sureButtonBackgroundColor: UIColor? {
        didSet {
    ...
    /// 确定按钮字体颜色，默认白色
    public var sureButtonTitleColor: UIColor? {
        didSet {
    ...
    /// 年 背景水印，设置无色可以隐藏
    public var yearPlaceholderColor: UIColor? {
        didSet {
    ...
    class func datePicker(style: DateStyle, scrollToDate: Date = Date(), completionHandler: @escaping (_ date: Date?) -> Void) -> DatePickerView {
    ...
    @objc func dismiss() {
        UIView.animate(withDuration: 0.3, animations: {
            ...
            self.backgroundColor = UIColor.init(hex: 0x000000, alpha: 0.0)
...
extension DatePickerView {
    private func setupUI() {
        ...
        backgroundColor = UIColor.init(hex: 0x000000, alpha: 0.0)
