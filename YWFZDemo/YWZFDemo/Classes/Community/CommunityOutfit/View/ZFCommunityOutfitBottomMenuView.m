//
//  ZFCommunityOutfitBottomMenuView.m
//  ZZZZZ
//
...
@interface ZFCommunityOutfitBottomMenuView()<WMMenuViewDelegate,WMMenuViewDataSource>
...
@implementation ZFCommunityOutfitBottomMenuView
...
- (NSString *)menuView:(WMMenuView *)menu titleAtIndex:(NSInteger)index {
    if (self.datasArray.count > index) {
        NSString *menuTitle = self.datasArray[index];
        if (!ZFIsEmptyString(menuTitle)) {
            return menuTitle;
        }
    }
    return @"";
...
- (void)setDatasArray:(NSArray *)datasArray {
    if (_datasArray != datasArray) {
        ...
        [_datasArray enumerateObjectsUsingBlock:^(NSString *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            
            NSString *title = ZFToString(obj);
...
- (WMMenuView *)menuView {
    if (!_menuView) {
        ...
        _menuView.lineColor = ZFC0x2D2D2D();
