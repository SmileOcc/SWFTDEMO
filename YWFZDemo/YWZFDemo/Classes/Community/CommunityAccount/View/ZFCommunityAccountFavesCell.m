...
@interface ZFCommunityAccountFavesCell () <ZFInitViewProtocol>
//以下为瀑布流子视图
...
@implementation ZFCommunityAccountFavesCell
...
- (void)zfAutoLayoutView {
    ...
    //图片高度
    CGFloat imageShowHeight = self.favesModel.twoColumnImageHeight;
 ...
 */
- (void)setFavesModel:(ZFCommunityFavesItemModel *)favesModel
{
    _favesModel = favesModel;
...
- (void)setupWaterFlowLayoutCellData
{
    ...
    if (_favesModel.reviewPic.count>0) {
        ZFCommunityPictureModel *picModel = _favesModel.reviewPic[0];
        waterPicUrl = ZFToString(picModel.bigPic);
    }
    
    if (!_favesModel.randomColor) {
        _favesModel.randomColor = [UIColor colorWithHexString:[ZFThemeManager randomColorString:nil]] ;
    }
    self.waterFlowPicImageView.backgroundColor = _favesModel.randomColor;
    
    //图片
    NSString *imagePaths = @"";
    if (_favesModel.reviewPic.count>0) {
        ZFCommunityPictureModel *reviewPicModel = _favesModel.reviewPic[0];
        imagePaths = ZFToString(reviewPicModel.bigPic);
    }
    [self.waterFlowPicImageView yy_setImageWithURL:[NSURL URLWithString:imagePaths]
                                      placeholder:nil];
    
    //帖子内容, 去掉回车与换行符
    NSString *conteentText = _favesModel.contentAttributedText.string;
    self.waterFlowContentLab.text = [conteentText replaceBrAndEnterChar];
    
    //头像
    self.waterFlowUserLab.text = ZFToString(_favesModel.nickname);
    [self.waterFlowUserImageView yy_setImageWithURL:[NSURL URLWithString:_favesModel.avatar]
    ...
    //用户是否点赞
    NSInteger likeNum = [_favesModel.likeCount integerValue];
    if (likeNum>0) {
        self.waterFlowLikeButton.selected = _favesModel.isLiked;
    ...
    if ([_favesModel.identify_type integerValue] > 0) {
        [self.waterFlowRankImageView yy_setImageWithURL:[NSURL URLWithString:_favesModel.identify_icon] options:kNilOptions];
...
- (void)likeButtonAction:(UIButton *)sender {
    if (self.communityFavesLikeCompletionHandler) {
        ...
        self.communityFavesLikeCompletionHandler(self.favesModel);
    }
}
