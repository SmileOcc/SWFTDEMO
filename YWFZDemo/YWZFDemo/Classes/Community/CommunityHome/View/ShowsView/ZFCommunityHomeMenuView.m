...
@interface ZFCommunityHomeMenuView()<WMMenuViewDelegate,WMMenuViewDataSource>
...
@implementation ZFCommunityHomeMenuView
...
- (NSString *)menuView:(WMMenuView *)menu titleAtIndex:(NSInteger)index {
    if (self.datasArray.count > index) {
        ZFCommunityChannelItemModel *itmeModel = self.datasArray[index];
        return ZFToString(itmeModel.cat_name);
    }
    return @"";
}
...
- (void)setDatasArray:(NSArray *)datasArray {
    if (_datasArray != datasArray) {
        ...
        [_datasArray enumerateObjectsUsingBlock:^(ZFCommunityChannelItemModel *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            
            NSString *title = ZFToString(obj.cat_name);
