...
@interface ZFCollectionPostsNormalView()
<ZFInitViewProtocol,
...
@implementation ZFCollectionPostsNormalView
...
- (void)requestPost:(BOOL)refresh {
    ...
    if ([AccountManager sharedManager].isSignIn) {
        @weakify(self)
        [self.viewModel requestCollectPostPageData:refresh type:@"0" completion:^(ZFCollectionPostListModel *listModel, NSArray *currentPageArray, NSDictionary *pageInfo) {
            @strongify(self)
...
- (ZFCollectionPostAnalyticsAOP *)analyticsAOP {
    if (!_analyticsAOP) {
        ...
        [_analyticsAOP baseConfigureSource:ZFAnalyticsAOPSourceDefault analyticsId:NSStringFromClass(ZFCollectionPostsNormalView.class)];
    }
