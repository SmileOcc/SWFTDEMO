...
//default values
NSString *const GGViewTrackerInitSwitchesNotification = @"GGViewTrackerManagerInitSwitchsNotification";
...
static GGViewTrackerManager *manager = nil;
static id<GGTrackerCommitProtocol> defaultProtocol = nil;

@interface GGViewTrackerManager ()
@property (nonatomic, strong) NSString *currentPageName;
@end

@implementation GGViewTrackerManager
+(instancetype)sharedManager
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[GGViewTrackerManager alloc] init];
    });
...
{
    if (self = [super init]) {
        _config = [[GGViewTrackerConfigModel alloc] init];
        _isDebugModeOn = NO;
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(TMVTM_handlerNotification:)
                                                     name:GGViewTrackerInitSwitchesNotification
                                                   object:nil];
    }
...
{
    [GGViewTrackerManager sharedManager].isDebugModeOn = YES;
}

+ (void)turnOffDebugMode
{
    [GGViewTrackerManager sharedManager].isDebugModeOn = NO;
}

+ (void)setCurrentPageName:(NSString*)pageName
{
    [GGViewTrackerManager sharedManager].currentPageName = pageName;
}

+ (NSString*)currentPageName
{
    NSString *current = [GGViewTrackerManager sharedManager].currentPageName;
    if (!current) {
        id delegate = [GGViewTrackerManager sharedManager].commitProtocol;
...
- (BOOL)clickNeedUploadWithWhiteList:(UIView*)view
{
    ...
    if (_config.clickSwitch) {
        id obj = [_config.clickWhiteList objectForKey:[GGViewTrackerManager currentPageName]];
...
{
    if (!view) return NO;
    return [self isPageNameInExposureWhiteList:[GGViewTrackerManager currentPageName]];
}
...
{
    if ([notify.name isEqualToString:GGViewTrackerInitSwitchesNotification]) {
...
{
    [GGExposureManager adjustStateForView:view forType:GGViewTrackerAdjustTypeForceExposure];
}
