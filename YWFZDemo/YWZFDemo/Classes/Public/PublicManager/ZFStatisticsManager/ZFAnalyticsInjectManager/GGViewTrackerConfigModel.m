...
@interface GGViewTrackerConfigModel ()
@property (nonatomic, strong) NSDictionary * propertyMap;
@end

@implementation GGViewTrackerConfigModel

- (instancetype)init
{
    if (self = [super init]) {
        _exposureUploadMode = GGExposureDataUploadModeNormal;
...
- (void)_initPropertyMap
{
    ...
    for (unsigned int i = 0; i < propertyCount; i++) {
        ...
        if (propertyName && propertyName.length ) {
            ...
            ViewTrackerPropertyModel *model = [GGViewTrackerConfigModel _getClassInfoFromTypeString:typeString forPropertyName:propertyName];
