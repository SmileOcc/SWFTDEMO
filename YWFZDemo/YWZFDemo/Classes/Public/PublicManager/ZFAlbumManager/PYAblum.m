...
- (void)setup {
    ...
    self.externalImportAssetModelArray = [[NSMutableArray alloc] init];
...
- (void)updateSelectAssetModelArray:(NSArray<PYAssetModel *> *)selectedArray {
    if (selectedArray) {
        [self.selectedAssetModelArray addObjectsFromArray:selectedArray];
    }
}
...
- (void)removeExternalImportAssetModelArray {
    [self.externalImportAssetModelArray removeAllObjects];
}
/**
 管理选中的assetModel
...
+ (NSMutableArray <PYAssetModel *>*)getSelectAssetArrayWithClickedModel: (PYAssetModel *)clickedModel andMaxCount:(NSInteger) maxCount andOverTopBlock: (void(^)(NSArray <PYAssetModel *>*modelArray, BOOL isVoerTop))block{
    ...
    [ablum getAssetWithImageWidth:ablum.selectedImageW andModel:clickedModel andBlock:^(PYAssetModel *assetFirstModel) {
        ...
        [assetLastModel getOriginImage:^(UIImage *image) {
...
- (NSArray<PYAssetModel *> *)getSelectAssetArrayWithAblumModel: (PYAblumModel *)ablumModel {
    
    NSArray <PYAssetModel *>* assetModelArray = [self.assetManager getAssetWithFetchResult:ablumModel.assetfetchResult];
    for (PYAssetModel *model in assetModelArray) {
        model.isShowMask = false;
    }
    self.currentAblumModel = ablumModel;
...
- (NSArray<PYAssetModel *> *)allPhotoAblumModelArray {
    if (!_allPhotoAblumModelArray_Private || !_allPhotoAblumModelArray_Private.count) {
        ...
        __weak typeof(self)weakSelf = self;
        PYAblumModel *ablumModel = [self.ablumManager getFetchResultWithAssetType:Enum_Image andAblumType:Enum_Ablum_AblumAll allBlum:^(NSArray<PYAblumModel *> *blumArray) {
            __strong typeof(self)strongSelf = weakSelf;
            strongSelf.fetchResultsArray = blumArray;
        }];
        self.currentAblumModel = ablumModel;
        NSArray <PYAssetModel *>* assetModelArray = [self.assetManager getAssetWithFetchResult:ablumModel.assetfetchResult];
        for (PYAssetModel *model in assetModelArray) {
...
- (NSArray <PYAblumModel*> *)fetchResultsArray {
    if (!_fetchResultsArray) {
        ///获取 fetchResult
        [self.ablumManager getFetchResultWithAssetType:Enum_Image andAblumType:Enum_Ablum_AblumAll allBlum:^(NSArray<PYAblumModel *> *blumArray) {
            self.fetchResultsArray = blumArray;
        }];
    }
    return _fetchResultsArray;
}
