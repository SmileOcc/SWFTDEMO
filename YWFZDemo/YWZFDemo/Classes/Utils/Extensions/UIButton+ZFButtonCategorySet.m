//
//  UIButton+ZFButtonCategorySet.m
//  ZZZZZ
//
...
static const void *ZFButtonBlockKey = &ZFButtonBlockKey;

@implementation UIButton (ZFButtonCategorySet)
                  ...
                  bageBorderColor:(UIColor *)borderColor
{
    if (numberValue>0) {
        ...
        self.imageView.badgeBgColor = ZFC0xFE5269();
        self.imageView.badgeTextColor = ZFCOLOR_WHITE;
        self.imageView.badgeFont = ZFFontBoldSize(10.0);
 ...
 */
- (void)zfLayoutStyle:(ZFButtonEdgeInsetsStyle)style imageTitleSpace:(CGFloat)space {
    ...
    switch (style) {
        case ZFButtonEdgeInsetsStyleTop: // image在上，label在下
        {
            ...
            break;
        case ZFButtonEdgeInsetsStyleLeft: // image在左，label在右
        {
            if ([SystemConfigUtils isRightToLeftShow]) {//阿语
            ...
            break;
        case ZFButtonEdgeInsetsStyleBottom: // image在下，label在上
        {
            if ([SystemConfigUtils isRightToLeftShow]) {//阿语
            ...
            break;
        case ZFButtonEdgeInsetsStyleRight: // image在右，label在左
        {
            if ([SystemConfigUtils isRightToLeftShow]) {//阿语
 ...
 */
-(void)addTouchUpInsideHandler:(ZFTouchedBlock)handler
{
    objc_setAssociatedObject(self, ZFButtonBlockKey, handler, OBJC_ASSOCIATION_COPY_NONATOMIC);
...
-(void)ok_touchUpInside:(UIButton *)btn{
    ZFTouchedBlock block = objc_getAssociatedObject(self, ZFButtonBlockKey);
    if (block) {
        block(btn);
    }
}
...
{
    [self setBackgroundImage:[UIImage zf_createImageWithColor:backgroundColor] forState:state];
}
