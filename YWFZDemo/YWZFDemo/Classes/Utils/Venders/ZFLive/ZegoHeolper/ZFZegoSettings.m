...
+ (instancetype)sharedInstance {
    static ZFZegoSettings *s_instance = nil;
...
- (ZegoUser *)getZegoUser
{
    ...
    user.userId = [ZFZegoSettings sharedInstance].userID;
    user.userName = [ZFZegoSettings sharedInstance].userName;
    
    return user;
}

- (NSUserDefaults *)myUserDefaults {
    return [[NSUserDefaults alloc] initWithSuiteName:@"group.ZZZZZ"];
}
...
- (NSString *)userID {
    if (_userID.length == 0) {
        ...
        if (userID.length > 0) {
            ...
            _userID = [NSString stringWithFormat:@"zezf%u", (unsigned)rand()];
