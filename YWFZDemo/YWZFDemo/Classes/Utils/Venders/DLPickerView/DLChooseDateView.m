...
@interface DLChooseDateView () <UIPickerViewDelegate, UIPickerViewDataSource>

@property (copy, nonatomic) NSString *title;
@property (copy, nonatomic) void(^cancelBlock)(void);
@property (copy, nonatomic) void(^confirmBlock)(NSString *date);
...
#define KPickerViewHeight   220

@implementation DLChooseDateView
                ...
                confirmBlock:(void(^)(NSString *date))confirmBlock
{
    ...
    return [[DLChooseDateView alloc] initWithFrame:rect
                                             title:title
                 ...
                 confirmBlock:(void(^)(NSString *date))confirmBlock
{
    ...
    if (self) {
        ...
        [control addTarget:self action:@selector(dismissDateView) forControlEvents:UIControlEventTouchUpInside];
...
- (void)configToolView {
    ...
    self.toolView.frame = CGRectMake(0, KScreenHeight, self.frame.size.width, (KToolViewHeight+KPickerViewHeight));
    self.toolView.backgroundColor = [UIColor whiteColor];
    [self addSubview:self.toolView];
    
    UIButton *saveBtn = [[UIButton alloc] init];
    saveBtn.frame = CGRectMake(self.frame.size.width - 70, 2, 60, 40);
    ...
    cancelBtn.frame = CGRectMake(10, 2, 60, 40);
    ...
    self.titleLbl.textColor = ColorHex_Alpha(0x999999, 1.0);
    self.titleLbl.font = ZFFontSystemSize(14);
...
- (void)configPickerView {
    self.pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.titleLbl.frame)-10, KScreenWidth, KPickerViewHeight)];
...
- (void)saveBtnClick {
    ...
    NSString *millisecond = @"00";
    self.selectStr = [NSString stringWithFormat:@"%ld-%@-%@  %@:%@:%@", [self.year integerValue], month, day, hour, minute, millisecond];
    
    if (self.confirmBlock) {
        self.confirmBlock(self.selectStr);
    }
...
- (void)showDateView:(BOOL)show {
    ...
    [UIView animateWithDuration:0.2f
                     ...
                     animations:^{
                         if (show) {
                             self.toolView.y = KScreenHeight-(KToolViewHeight+KPickerViewHeight);
                             [self show];
                         } else {
                             self.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0];
                             self.toolView.y = KScreenHeight;
                         }
...
- (int)compareDate:(NSString *)date01 withDate:(NSString *)date02{
    ...
    switch (result) {
        ...
        default: YWLog(@"erorr dates %@, %@", dt2, dt1);break;
    }
    return ci;
}
