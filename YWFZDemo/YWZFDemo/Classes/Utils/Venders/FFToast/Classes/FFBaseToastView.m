...
-(void)layoutToastView{
    ...
    switch (self.toastType) {
        ...
        case FFToastTypeSuccess: {
            ...
            if (!_iconImage) {
                self.iconImage = [self.class getToastImageWithName:@"fftoast_success"];
            }
            break;
        }
        case FFToastTypeError: {
            ...
            if (!_iconImage) {
                self.iconImage = [self.class getToastImageWithName:@"fftoast_error"];
            }
            break;
        }
        case FFToastTypeWarning: {
            ...
            if (!_iconImage) {
                self.iconImage = [self.class getToastImageWithName:@"fftoast_warning"];
            }
            break;
        }
        case FFToastTypeInfo: {
            ...
            if (!_iconImage) {
                self.iconImage = [self.class getToastImageWithName:@"fftoast_info"];
            }
            break;
        }
...
+ (UIImage *)getToastImageWithName:(NSString*)name {
...
-(CGRect)toastViewFrame{
    ...
    if (self.iconImage == nil) {
        textMaxWidth = KScreenWidth - 2*HORIZONTAL_SPACE;
    }else{
        textMaxWidth = KScreenWidth - _iconImageSize.width - 3*HORIZONTAL_SPACE;
    }
    ...
    switch (self.toastPosition) {
        ...
        case FFToastPositionBelowStatusBarWithFillet: {
            ...
            toastViewY = STATUSBAR_HEIGHT;
            toastViewW = KScreenWidth - 2*HORIZONTAL_SPACE;
