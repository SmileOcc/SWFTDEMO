...
- (void)changeActivity:(BOOL)active atIndex:(NSInteger)index
{
    if (self.dotViewClass) {
        ...
        // V4.9.0 产品@刘坤 说要修改小圆点大小 add by: mwx
        if (CGSizeEqualToSize(self.dotSize, CGSizeZero)) return;
        CGFloat lastDotX = 0;
        for (NSInteger i=0; i<self.dots.count; i++) {
            UIImageView *tmpDotView = self.dots[i];
            if (![tmpDotView isKindOfClass:[UIImageView class]]) return;

            CGFloat dotSize = (i == index) ? 11 : 7;
            CGFloat dotY = (i == index) ? -(11-7)/2 : 0;
            
            tmpDotView.frame = CGRectMake(lastDotX, dotY, dotSize, dotSize);
            lastDotX = CGRectGetMaxX(tmpDotView.frame) + 8;
        }
    }
}
