...
#pragma mark  ////occ修改
    ...
    if (_scrollDirection == UICollectionViewScrollDirectionVertical) {
        if (self.titlesAttrGroup.count > 0 || self.titlesGroup.count > 0) {
...
- (void)setIsEnableScroll:(BOOL)isEnableScroll
{
    _isEnableScroll = isEnableScroll;
    self.mainView.scrollEnabled = _isEnableScroll;
}
...
//        y = [SystemConfigUtils isRightToLeftShow] ? _pageControlBottomOffset : self.mainView.sd_height - size.height - _pageControlBottomOffset;//10;
        y = self.mainView.sd_height - size.height - _pageControlBottomOffset;//10;

    }
    //CGFloat y = self.mainView.sd_height - size.height - 10;  //源
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
                                   ...
                                   placeholder:self.placeholderImage
                                      ...
                                      transform:^UIImage *(UIImage *image, NSURL *url) {
                                        if ([image isKindOfClass:[YYImage class]]) {
                                            ...
                                            if (showImage.animatedImageType == YYImageTypeGIF
                                                || showImage.animatedImageData) {
                                                return image;
                                            }
                                        }
    ...
    if (_titlesGroup.count && itemIndex < _titlesGroup.count) {
        cell.attributedString = _titlesGroup[itemIndex];
    }
...
#pragma mark ////occ修改
    // 只针对文字处理
    if (self.titlesGroup.count > 0 || self.titlesAttrGroup.count > 0) {
