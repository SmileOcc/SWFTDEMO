...
- (void)setItem:(YYPhotoGroupItem *)item {
    ...
    [_imageView yy_setImageWithURL:item.largeImageURL
                       placeholder:item.thumbImage options:kNilOptions progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        @strongify(self);
        if (!self) return;
...
- (void)resizeSubviewSize {
    _imageContainerView.mj_origin = CGPointZero;
...
- (instancetype)initWithGroupItems:(NSArray *)groupItems {
    ...
    _pager.height = 10;
    _pager.center = CGPointMake(self.width / 2, self.height - 30);
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    if (self.dismissCompletion) { //商详使用时不要背景图
        ...
        self.background.backgroundColor = [UIColor colorWithWhite:0 alpha:1];
        self.blurBackground.backgroundColor = [UIColor colorWithWhite:0 alpha:1];
        self.dismissCompletion(self.currentPage);
    }
    if (self.startDismissCompletion) {//帖子详情回调
        self.startDismissCompletion(self.currentPage);
    }
...
//#warning -mark 此处为处理查看大图时，取消的动画效果
    [UIView animateWithDuration:animated ? 0.4 : 0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (self.dismissCompletion) {
            ...
            self.blurBackground.alpha = 0.0;
            self.background.backgroundColor = [UIColor colorWithWhite:0 alpha:0.2];
            self.blurBackground.backgroundColor = [UIColor colorWithWhite:0 alpha:0.2];
        }
        if (isFromImageClipped) {
...
//            [actionsheet showInView:self];
            ...
            //bug: 长按图片时上面的弹框会被图片盖住采用自定义的
            NSString *otherTitle = ZFLocalizedString(@"Save_Picture",nil);
            ...
            [ZFActionSheetView actionSheetByBottomCornerRadius:^(NSInteger buttonIndex, id title) {
                [self actionSheetClickedButton];
            } cancelButtonBlock:nil sheetTitle:nil cancelButtonTitle:cancelTitle otherButtonTitleArr:@[otherTitle]];
        }
    }
...
- (void)alertMessage:(id)info {
    ...
    ShowToastToViewWithText(nil, info);
}
