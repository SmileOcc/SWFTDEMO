...
/// 标题数组 默认 缓存 key 为 title 可通过数据源代理 进行替换
@property (nonatomic, strong) NSMutableArray<NSString *> *subTitlesM;
/// 菜单栏
...
/// 页面ScrollView
@property (nonatomic, strong, readonly) YNPageScrollView *pageScrollView;
/// 头部headerView
...
/// 头部伸缩背景View (add by : YW)
@property (nonatomic, copy) void (^topViewHasDidScrollView)(CGFloat offsetY);
