//
//  UIScrollView+ZFBlankPageView.h
//  CommonFrameWork
//
...
/** 进入刷新状态的回调 */
typedef void (^ZFRefreshingBlock)(void);
/** 下拉显示Banner Block */
...
} ZFBlankPageViewStatus;

@interface UIScrollView (ZFBlankPageView)
...
/** 自定义按钮点击的事件 */
@property (nonatomic, copy) void (^blankPageViewActionBlcok)(ZFBlankPageViewStatus status);

/**
 * 一个属性即可自动设置显示请求提示view,(但是在请求失败时只能显示无数据提示)
 * 此方法一定要放在请求数据回来时的reloadData之前设置此属性效果才正常
 */
//@property (nonatomic, assign) BOOL automaticShowTipView;//目前ZZZZZ项目中没有使用该属性,暂时关闭
