//
//  UIScrollView+ZFBlankPageView.m
//  CommonFrameWork
//
//  Created by YW on 2017/4/17.
...
/** 网络连接失败 */
#define kNetworkConnectDefaultFailTips ZFLocalizedString(@"BlankPage_NetworkError_tipTitle",nil)
/** 刷新提示文案 */
#define kAgainRequestDefaultTipString  ZFLocalizedString(@"EmptyCustomViewManager_refreshButton",nil)
/** 请求失败默认提示文案 */
#define kReqFailDefaultTipText         ZFLocalizedString(@"EmptyCustomViewManager_titleLabel",nil)
/** 请求空数据默认提示文案 */
#define kEmptyDataDefaultTipText       ZFLocalizedString(@"EmptyCustomViewHasNoData_titleLabel",nil)
/* 弱引用 */
...
static char const * const kNetworkErrorBtnTitleKey      = "kNetworkErrorTitleKey";

@implementation UIScrollView (ZFBlankPageView)
...
- (UIImage *)emptyDataImage
{
    ...
    if (!image) {
        image = [UIImage imageNamed:@"blankPage_noCart"];
    }
    return image;
}
...
- (UIImage *)requestFailImage
{
    ...
    if (!image) {
        image = [UIImage imageNamed:@"blankPage_requestFail"];
    }
    return image;
}
...
- (void)setBlankPageViewActionBlcok:(void (^)(ZFBlankPageViewStatus))blankPageViewActionBlcok
{
    objc_setAssociatedObject(self, kBlankPageViewActionBlcokKey, blankPageViewActionBlcok, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (void (^)(ZFBlankPageViewStatus))blankPageViewActionBlcok
{
 ...
 */
- (void)addCommunityHeaderRefreshBlock:(ZFRefreshingBlock)headerBlock
           footerRefreshBlock:(ZFRefreshingBlock)footerBlock
              startRefreshing:(BOOL)startRefreshing
{
    if (headerBlock) {
        @weakify(self)
        ZFCommunityRefreshHeader *header = [ZFCommunityRefreshHeader headerWithRefreshingBlock:^{
            @strongify(self)
...
/**
 ...
 @param startRefreshing 是否需要立即刷新
 */
- (void)addHeaderRefreshBlock:(ZFRefreshingBlock)headerBlock
           footerRefreshBlock:(ZFRefreshingBlock)footerBlock
              startRefreshing:(BOOL)startRefreshing
{
    if (headerBlock) {
...
- (void)showRequestTip:(NSDictionary *)totalPageCurrentPageCountDict
{
    ...
    if ([self contentViewIsEmptyData]) {//页面没有数据
        ...
        if (![AFNetworkReachabilityManager sharedManager].reachable) {
            ...
            ZFBlankPageViewStatus status = requestSuccess ? RequestEmptyDataStatus : RequestFailStatus;
        ...
        if (!requestSuccess && self.mj_header) {
            ...
            //ShowToastToViewWithText(vcView, ZFLocalizedString(@"EmptyCustomViewManager_titleLabel",nil));
        }
    }
}
 ...
 * 设置提示图片和文字
 */
- (void)showTipWithStatus:(ZFBlankPageViewStatus)state
{
    //先移除页面上已有的提示视图
    [self removeOldTipBgView];
    ...
    if (state == RequesNoNetWorkStatus) {//没有网络
        ...
        actionBtnTitle = self.requestFailBtnTitle ? : kAgainRequestDefaultTipString;
        if (self.blankPageViewActionBlcok) {
            actionBtnBlock = blankPageViewBtnActionBlcok;
    ...
    ZFBlankPageTipView *tipBgView = [ZFBlankPageTipView tipViewByFrame:rect
                                                           moveOffsetY:tipTopY
                                                              topImage:tipImage
                                                                 title:tipString
...
- (void)removeOldTipBgView
{
    for (UIView *tempView in self.subviews) {
        if ([tempView isKindOfClass:[ZFBlankPageTipView class]] &&
...
//
//+ (void)zf_exchangeInstanceMethod:(SEL)originSelector otherSelector:(SEL)otherSelector
//{
...
//{
//    //交换刷新表格方法
//    [self zf_exchangeInstanceMethod:@selector(reloadData)
//                      otherSelector:@selector(zf_reloadData)];
//    //交换删除表格方法
//    [self zf_exchangeInstanceMethod:@selector(deleteRowsAtIndexPaths:withRowAnimation:)
//                      otherSelector:@selector(zf_deleteRowsAtIndexPaths:withRowAnimation:)];
//    //交换刷新表格Sections方法
//    [self zf_exchangeInstanceMethod:@selector(reloadSections:withRowAnimation:)
//                      otherSelector:@selector(zf_reloadSections:withRowAnimation:)];
//}
//
//- (void)zf_reloadData
//{
...
//}
//
//- (void)zf_deleteRowsAtIndexPaths:(NSArray<NSIndexPath *> *)indexPaths
//                 withRowAnimation:(UITableViewRowAnimation)animation
//{
//    [self zf_deleteRowsAtIndexPaths:indexPaths withRowAnimation:animation];
//    [self convertShowTipView];
//}
//
//- (void)zf_reloadSections:(NSIndexSet *)sections
//         withRowAnimation:(UITableViewRowAnimation)animation
//{
//    [self zf_reloadSections:sections withRowAnimation:animation];
//    [self convertShowTipView];
//}
//@end
//
...
//                      otherSelector:@selector(zf_reloadData)];
//
//    [self zf_exchangeInstanceMethod:@selector(deleteSections:)
//                      otherSelector:@selector(zf_deleteSections:)];
//
//    [self zf_exchangeInstanceMethod:@selector(reloadSections:)
//                      otherSelector:@selector(zf_reloadSections:)];
//
//    [self zf_exchangeInstanceMethod:@selector(deleteItemsAtIndexPaths:)
//                      otherSelector:@selector(zf_deleteItemsAtIndexPaths:)];
//
//    [self zf_exchangeInstanceMethod:@selector(reloadItemsAtIndexPaths:)
//                      otherSelector:@selector(zf_reloadItemsAtIndexPaths:)];
//}
//
//- (void)zf_reloadData
//{
...
//}
//
//- (void)zf_deleteSections:(NSIndexSet *)sections
//{
//    [self zf_deleteSections:sections];
//    [self convertShowTipView];
//}
//
//- (void)zf_reloadSections:(NSIndexSet *)sections
//{
//    [self zf_reloadSections:sections];
//    [self convertShowTipView];
//}
//
//- (void)zf_deleteItemsAtIndexPaths:(NSArray<NSIndexPath *> *)indexPaths
//{
//    [self zf_deleteItemsAtIndexPaths:indexPaths];
//    [self convertShowTipView];
//}
//
//- (void)zf_reloadItemsAtIndexPaths:(NSArray<NSIndexPath *> *)indexPaths
//{
//    [self zf_reloadItemsAtIndexPaths:indexPaths];
//    [self convertShowTipView];
//}
//@end
