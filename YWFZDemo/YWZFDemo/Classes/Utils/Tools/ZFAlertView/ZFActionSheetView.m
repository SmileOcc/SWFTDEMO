...
// 按钮标题颜色 (默认为系统 UIAlertView的蓝色)
#define OKActionSheet_BtnTitleNormalColor               OkColorFromHex(0x2F7AFF)
// 按钮不可用标题颜色
#define OKActionSheet_BtnTitleDisabledColor             OkColorFromHex(0xE7E7E7)
// 主色 (默认为系统 UIAlertView的蓝色)
#define OKActionSheet_canceBtnlTitleNormalColor         OkColorFromHex(0x2F7AFF)
// 灰色线条颜色
#define OKActionSheet_LineColor                         OkColorFromHex(0xDDDDDD)
// 按钮不可用状态背景颜色
#define OKActionSheet_BtnDIsableBgColor                 OkColorFromHex(0xe2e2e2)
// 按钮高亮状态背景颜色
#define OKActionSheet_BtnHighlightedBgColor             [OkColorFromHex(0xe8e8e8) colorWithAlphaComponent:0.5]
// 按钮超过最大个数,则滚动
#define OKActionSheet_MaxButtonCount                    4
// 控件离屏幕边缘上下左右间距
#define OKActionSheet_LineSpacing                       10
// 按钮高度
#define OKActionSheet_ButtonHeight                      58.0f
// 按钮间隙
#define OKActionSheet_CancelBtnSpace                    10.0f
// 标记取消按钮tag
#define OKActionCancelBtnTag				            2017
// 圆角
#define OKActionCornerRadius				            12
...
@interface ZFActionSheetView ()<UIScrollViewDelegate>
...
@implementation ZFActionSheetView
		 ...
		 actionSheetStyleType:(CCActionSheetStyleType)styleType
{
	...
	if (self) {
		...
		//设置按钮标题主色
		self.titleTextAttributes = [ZFActionSheetView appearance].titleTextAttributes ? : nil;
		self.otherBtnTitleAttributes = [ZFActionSheetView appearance].otherBtnTitleAttributes ? : nil;
		self.themeColorBtnTitleAttributes = [ZFActionSheetView appearance].themeColorBtnTitleAttributes ? : nil;
...
- (void)removeOKActionSheetFromWindow
{
	...
	for (UIView *windowSubView in window.subviews) {
		if ([windowSubView isKindOfClass:[ZFActionSheetView class]]) {
					...
					buttonImageArr:(NSArray *)buttonImageArr
{
	...
	for (int i = 0 ; i<allbuttontitleArr.count; i++) {
		...
		actionBtn.tag = i;
		[actionBtn.titleLabel setFont:OKActionSheet_font(18)];
		[actionBtn addTarget:self action:@selector(actionBtnAction:) forControlEvents:UIControlEventTouchUpInside];
		[actionBtn setTitleColor:OKActionSheet_BtnTitleDisabledColor forState:UIControlStateDisabled];
		[actionBtn setTitleColor:OKActionSheet_BtnTitleNormalColor forState:0];
		[actionBtn setBackgroundImage:[ZFActionSheetView ok_imageWithColor:OKActionSheet_BtnDIsableBgColor] forState:UIControlStateDisabled];
		[actionBtn setBackgroundImage:[ZFActionSheetView ok_imageWithColor:OKActionSheet_BtnHighlightedBgColor] forState:UIControlStateHighlighted];
					   ...
					   buttonTitleArr:(NSArray *)buttonTitleArr
{
	...
	if (title)
		...
		CGFloat fontSize = [ZFActionSheetView calculateTextHeight:titleLab.font constrainedToWidth:titleLabW textObject:titleObject];
	...
	[cancelBtn.titleLabel setFont:OKActionSheet_font(18)];
	[cancelBtn addTarget:self action:@selector(actionBtnAction:) forControlEvents:UIControlEventTouchUpInside];
	[cancelBtn setTitleColor:OKActionSheet_BtnTitleDisabledColor forState:UIControlStateDisabled];
	[cancelBtn setTitleColor:OKActionSheet_BtnTitleNormalColor forState:0];
	[cancelBtn setBackgroundImage:[ZFActionSheetView ok_imageWithColor:OKActionSheet_BtnDIsableBgColor] forState:UIControlStateDisabled];
	[cancelBtn setBackgroundImage:[ZFActionSheetView ok_imageWithColor:[UIColor groupTableViewBackgroundColor]] forState:UIControlStateHighlighted];
	...
	if ([cancelTitle isKindOfClass:[NSString class]]) {
		...
		if (self.themeColorBtnTitleAttributes) {
			...
			[cancelBtn setTitleColor:OKActionSheet_canceBtnlTitleNormalColor forState:0];
							 ...
							 buttonTitleArr:(NSArray *)buttonTitleArr
{
	...
	for (int i = 0 ; i<allbuttontitleArr.count; i++) {
		...
		if (cancelTitle && i == allbuttontitleArr.count-1)
			...
			[actionBtn setTitleColor:OKActionSheet_canceBtnlTitleNormalColor forState:0];
        ...
        CGFloat space = (isPhoneX && i == allbuttontitleArr.count-1) ? 0 : OKActionSheet_LineSpacing;
		contentView.height = CGRectGetMaxY(actionBtn.frame) + space;
	}

	//创建滚动底下提示文字
	if (allbuttontitleArr.count > OKActionSheet_MaxButtonCount) {
        scrollView.showsVerticalScrollIndicator = YES;
        BOOL scroll = [[NSUserDefaults standardUserDefaults] boolForKey:@"alreadyScroll"];
        if (!scroll) {
            [UIView animateWithDuration:0.5 animations:^{
                [scrollView setContentOffset:CGPointMake(0, OKActionSheet_ButtonHeight)];
            }completion:^(BOOL finished) {
                [UIView animateWithDuration:0.5 animations:^{
                    [scrollView setContentOffset:CGPointMake(0, 0)];
                }];
            }];
            [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"alreadyScroll"];
        }
								 ...
								 buttonImageArr:(NSArray *)buttonImageArr
{
	...
	NSInteger onePageItemCount = OKActionSheet_MaxButtonCount;//一页的最多5数
	...
	[cancelBtn setTitleColor:OKActionSheet_BtnTitleNormalColor forState:0];
	[cancelBtn setBackgroundImage:[ZFActionSheetView ok_imageWithColor:OKActionSheet_BtnDIsableBgColor] forState:UIControlStateDisabled];
	[cancelBtn setBackgroundImage:[ZFActionSheetView ok_imageWithColor:OKActionSheet_BtnHighlightedBgColor] forState:UIControlStateHighlighted];
	[contentView addSubview:cancelBtn];
	[cancelBtn setExclusiveTouch:YES];
...
- (void)dismissCCActionSheet:(id)sender
{
	...
	if (self.actionSheetStyleType == TopSquareSheetStyle) {
		...
		contentViewY = kOkFullScreenHeight + kOkHomeBarHeight;
	}
