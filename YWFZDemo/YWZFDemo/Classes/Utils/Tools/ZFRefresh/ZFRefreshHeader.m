...
/** ZF刷新控件的状态 */
typedef NS_ENUM(NSInteger, ZFRefreshState) {
    /** 松开就可以进行刷新的状态 */
    ZFRefreshStateBannerPulling=10,
    /** Banner即将刷新的状态 */
    ZFRefreshStateBannerWillRefresh,
    /** 触发 下拉banner */
    ZFRefreshStateDropBannerTriggered,
    /** 显示 下拉banner,类似淘宝二楼效果 */
    ZFRefreshStateDropBannerShowing
};
...
- (void)setBannerModel:(ZFBannerModel *)bannerModel{
    ...
    self.hideTitle = ZFIsEmptyString(_bannerModel.image);
    
    if (!self.scrollView.showDropBanner || ZFIsEmptyString(_bannerModel.image))  {
        [_bannerView removeFromSuperview];
        self.bannerView = nil;

        self.tipLabel.attributedText = nil;
        self.tipLabel.font = ZFFontSystemSize(12);
        self.tipLabel.textColor = ZFCOLOR(153, 153, 153, 1);
        self.tipLabel.text = ZFLocalizedString(@"community_showoutfit_title", nil);
    ...
    [self.bannerView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo([bannerModel.banner_height floatValue] * KScreenWidth /[bannerModel.banner_width floatValue]);
    }];
...
- (NSMutableAttributedString *)queryTitleAttribute:(NSString *)title {
    ...
    titleAttribute.yy_font = ZFFontSystemSize(12);
    ...
    if (self.scrollView.showDropBanner) {
        if ([self.scrollView isDragging]) {
            if (self.state == MJRefreshStatePulling && offsetY < dropBannerOffsetY) {
                YWLog(@"触发下拉banner");
                self.state = ZFRefreshStateDropBannerTriggered;
            } else if (self.state == ZFRefreshStateDropBannerTriggered && offsetY >= dropBannerOffsetY) {
                self.state = MJRefreshStateIdle;
            }
        }else{
            if (self.state == ZFRefreshStateDropBannerTriggered) {
                self.state = ZFRefreshStateDropBannerShowing;
            }else if (self.state == ZFRefreshStateDropBannerShowing) {
                self.state = MJRefreshStateIdle;
            }
        }
    }
    ...
    switch (state) {
        case MJRefreshStateIdle:
        {
            ...
            if (self.isCommunityRefresh && !self.hideTitle) {
                attribute = [self queryTitleAttribute:ZFLocalizedString(@"refresh_pull_down", nil)];
            }else{
                if (!self.hideTitle) {
                    attribute = [self queryTitleAttribute:ZFLocalizedString(@"refresh_pull_down", nil)];
                }
        ...
        case MJRefreshStateRefreshing:
        {
            ...
            if (self.isCommunityRefresh && !self.hideTitle) {
                attribute = [self queryTitleAttribute:ZFLocalizedString(@"refresh_loading", nil)];
            }else{
                if (!self.hideTitle) {
                    attribute = [self queryTitleAttribute:ZFLocalizedString(@"refresh_loading", nil)];
                }
            ...
            break;
        case ZFRefreshStateDropBannerTriggered:
        {
            if (self.isCommunityRefresh && !self.hideTitle) {
                attribute = [self queryTitleAttribute:ZFLocalizedString(@"refresh_release", nil)];
            }else{
                if (!self.hideTitle) {
                    attribute = [self queryTitleAttribute:ZFLocalizedString(@"refresh_release", nil)];
                }
            ...
            break;
        case ZFRefreshStateDropBannerShowing:
        {
