...
//@property (nonatomic, strong) OSSVMessageVC                        *messageVC;
//@property (nonatomic, strong) OSSVNewChannelViewController              *channelNewVC;
@property (nonatomic, strong) OSSVCartVC                       *cartVC;
...
+ (void)initialize {
    ...
    [[UITabBar appearance] setUnselectedItemTintColor:[OSSVThemesColors col_999999]];
    [[UITabBarItem appearance] setTitleTextAttributes:@{NSForegroundColorAttributeName : [OSSVThemesColors col_999999],
                                                        NSFontAttributeName : [UIFont systemFontOfSize:10]}
                                                        forState:UIControlStateNormal];
    [[UITabBarItem appearance] setTitleTextAttributes:@{NSForegroundColorAttributeName : [OSSVThemesColors col_262626],
                                                        NSFontAttributeName : [UIFont systemFontOfSize:10]}
                                                        forState:UIControlStateSelected];
    [[UITabBarItem appearance] setBadgeColor:OSSVThemesColors.col_B62B21];
}
...
- (void)viewDidLoad {
    ...
    self.tabBar.layer.shadowColor = [OSSVThemesColors col_EEEEEE].CGColor;
    ...
    self.tabBar.backgroundColor =  [OSSVThemesColors stlWhiteColor];
    self.tabBar.translucent = NO; // 设置不半透明
    
    // 购物车数量
    NSInteger allGoodsCount = [[OSSVCartsOperateManager sharedManager] cartValidGoodsAllCount];
    if (allGoodsCount > 0) {
        self.viewControllers[STLMainMoudleCart].tabBarItem.badgeValue = [NSString stringWithFormat:@"%lu",(unsigned long)allGoodsCount];
        if (allGoodsCount > 99) {
            allGoodsCount = 99;
            self.viewControllers[STLMainMoudleCart].tabBarItem.badgeValue = [NSString stringWithFormat:@"%lu+",(unsigned long)allGoodsCount];
...
- (CAShapeLayer *)line {
    ...
    [solidShapeLayer setStrokeColor:[[OSSVThemesColors col_EEEEEE] CGColor]];
...
- (void)setupChildViewController:(UIViewController *)childVc title:(NSString *)title imageName:(NSString *)imageName selectedImageName:(NSString *)selectedImageName {
    ...
    OSSVNavigationVC *nav = [[OSSVNavigationVC alloc] initWithRootViewController:childVc];
...
- (OSSVNavigationVC *)navigationControllerWithMoudle:(STLMainMoudle)moudle {
    if (self.viewControllers.count > moudle) {
        OSSVNavigationVC *nav = [self.viewControllers objectAtIndex:moudle];
        return nav;
    } else {
        return nil;
    }
...
//    if ([viewController.topViewController isKindOfClass:OSSVMessageVC.class] && ![OSSVAccountsManager sharedManager].isSignIn) {
...
- (void)changeValue:(NSNotification *)notify {
    ...
    tabBarItem =  [self.viewControllers[STLMainMoudleCart] tabBarItem];

    tabBarItem.badgeColor = [OSSVThemesColors stlWhiteColor];

    // 购物车数量
    NSInteger allGoodsCount = [[OSSVCartsOperateManager sharedManager] cartValidGoodsAllCount];
    if (allGoodsCount > 0) {
        tabBarItem.badgeColor = [OSSVThemesColors col_B62B21];
        if (allGoodsCount > 99) {
            allGoodsCount = 99;
...
//                lbl.frame = CGRectMake(25*DSCREEN_WIDTH_SCALE - 7, 5, 8, 8);
                lbl.frame = CGRectMake(30*DSCREEN_WIDTH_SCALE - 7, 5, 8, 8);

            } else {
//                lbl.frame = CGRectMake(itemWidth*(STLMainMoudleAccount+1)-25*DSCREEN_WIDTH_SCALE, 5, 8, 8);
                lbl.frame = CGRectMake(itemWidth*(STLMainMoudleAccount+1)-30*DSCREEN_WIDTH_SCALE, 5, 8, 8);
            }
