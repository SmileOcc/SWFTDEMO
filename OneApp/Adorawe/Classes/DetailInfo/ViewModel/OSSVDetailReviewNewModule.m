...
@interface OSSVDetailReviewNewModule ()

@property (nonatomic, assign) CGFloat bottomOffset;

@end

@implementation OSSVDetailReviewNewModule
...
-(NSArray *)childRowsCalculateFramesWithBottomOffset:(CGFloat)bottomoffset section:(NSInteger)section
{
    ...
    if (rows) {
        ...
        CGFloat height = 0;
        CGFloat yOffset = bottomoffset;
        if (!CGSizeEqualToSize(self.cellSize, CGSizeZero) && self.cellSize.height > 0) {
            height = self.cellSize.height;
            yOffset = bottomoffset + kPaddingGoodsDetailSpace;
        }
