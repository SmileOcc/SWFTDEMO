...
@interface OSSVDetailCollectionLayout ()
...
@implementation OSSVDetailCollectionLayout

- (void)dealloc
{
    STLLog(@"OSSVDetailCollectionLayout dealloc");
}
...
-(void)prepareLayout
{
    ...
    if (_reloadSection >= 0) {
        ...
        STLLog(@"allAttributesItemList.count -%ld", self.allAttributesItemList.count);
        STLLog(@"removeAllRange length -%ld, location -%ld", removeAllRange.length, removeAllRange.location);
        ...
        STLLog(@"attributesList.count -%ld", self.sectionAttributesList.count);
        STLLog(@"removeRange length -%ld, location -%ld", removeRange.length, removeRange.location);
        ...
        STLLog(@"columnHeights.count -%ld", self.columnHeights.count);
        STLLog(@"columnHeightRemoveRange length -%ld, location -%ld", columnHeightRemoveRange.length, columnHeightRemoveRange.location);
    ...
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(customerLayoutDatasource:sectionNum:)]) {
        for (NSInteger i = startSection; i < sections; i++) {
            ...
            id <OSSVCollectionSectionProtocol> sectionModule = [self.dataSource customerLayoutDatasource:self.collectionView sectionNum:i];
            ...
            if ([self.dataSource respondsToSelector:@selector(customerLayoutSectionHeightHeight:layout:indexPath:)]) {
                ...
                if (sectionHeight) {
                    //section header
                    UICollectionViewLayoutAttributes *headerAttributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:DetailCustomerLayoutHeader withIndexPath:indexPath];
...
//            }
            ...
            if ([self.dataSource respondsToSelector:@selector(customerLayoutSectionFooterHeight:layout:indexPath:)]) {
                ...
                if (sectionFooter) {
                    //section footer
                    UICollectionViewLayoutAttributes *headerAttributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:DetailCustomerLayoutFooter withIndexPath:indexPath];
                    CGFloat lastSectionHeight = [[self.columnHeights lastObject] floatValue];
