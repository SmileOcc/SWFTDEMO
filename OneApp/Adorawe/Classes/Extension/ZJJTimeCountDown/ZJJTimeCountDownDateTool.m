...
+ (long long)getTimeTampWithStr:(NSString *)str timeStyle:(ZJJCountDownTimeStyle)timeStyle{
    ...
    if (timeStyle == ZJJcountDownSecondStyle) {
        return str.longLongValue;
    }
    if (timeStyle == ZJJcountDownSecondStyle) {
        NSTimeInterval aTimeInterval = [NSDate timeIntervalSinceReferenceDate] + str.longLongValue;
        NSDate *newDate = [NSDate dateWithTimeIntervalSinceReferenceDate:aTimeInterval];
        ///由于后台传回来的是倒计时90分钟的秒数，所以serverEndTime需要加上当前时间
