...
- (instancetype)initWithScrollView:(UIScrollView *)scrollView dataList:(NSMutableArray *)dataList{
    ...
    if (self) {
        ...
        _cellManager.labelInContentView = self.labelInContentView;
...
- (void)deleteTimeLabel:(ZJJTimeCountDownLabel *)label
{
    if ([self.labelManager.timeLabelDataList containsObject:label]) {
        [self.labelManager.timeLabelDataList removeObject:label];
    }
}
...
- (NSAttributedString *)timeStringWithModel:(id)model timeLabel:(ZJJTimeCountDownLabel *)timeLabel{
    ...
    NSString *serverEndTime = [[model valueForKey:timeLabel.timeKey] copy];
    if (self.timeStyle == ZJJcountDownSecondStyle) {
        NSInteger timeValue = serverEndTime.longLongValue;
        timeValue -= 1;
        [model setValue:[NSString stringWithFormat:@"%ld", timeValue] forKey:@"time"];
        serverEndTime = [[model valueForKey:timeLabel.timeKey] copy];
    }
...
- (NSAttributedString *)getNowTimeWithStartTimeTamp:(NSInteger )startTimeTamp endTimeTamp:(long long)endTimeTamp timeLabel:(ZJJTimeCountDownLabel *)timeLabel{
    ...
    NSTimeInterval timeInterval = endTimeTamp;
    if (self.timeStyle != ZJJcountDownSecondStyle) {
        timeInterval = endTimeTamp - startTimeTamp;
    }
