...
@property (nonatomic, weak) id<OSSVAccountsVCProtocol>                 accountVCProtocol;
@property (nonatomic, strong) OSSVAccountTablesMenuView                *menuView;
@property (nonatomic, strong) OSSVAccountsTableFootView              *tableFooterView;
...
- (void)refreCategoryItemCell {
    for (NSInteger i=0; i<self.sectionTypeModelArr.count; i++){
        OSSVAccountHeadCellTypeModel *sectionModel = self.sectionTypeModelArr[i];
...
- (void)reloadAccountTableView {
    CGFloat totalHeight = [OSSVAccountsHeadView accountHeaderContentH:self.needShowBanner];
...
//
    if (!self.topBackgroudColorView) {
        ...
        self.topBackgroudColorView.backgroundColor = [OSSVThemesColors col_0D0D0D];
...
- (void)setSectionTypeModelArr:(NSArray<OSSVAccountHeadCellTypeModel *> *)sectionTypeModelArr {
    _sectionTypeModelArr = sectionTypeModelArr;
    [self reloadAccountTableView];
}

- (CGFloat)heightForFooter:(NSInteger)section {
    OSSVAccountHeadCellTypeModel *sectionModel = self.sectionTypeModelArr[section];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    OSSVAccountHeadCellTypeModel *sectionModel = self.sectionTypeModelArr[indexPath.section];
    OSSVAccountHeadBaseCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass(sectionModel.sectionRowCellClass) forIndexPath:indexPath];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.canScroll) {
        if (scrollView.contentOffset.y >= headerOffsetY) {
            ...
            _menuView.backgroundColor = [OSSVThemesColors stlWhiteColor];
        } else {
            ...
            if (app_type == 3) {
                ...
                _menuView.backgroundColor = [OSSVThemesColors col_F5F5F5];
            }
        }
    } else {
        scrollView.contentOffset = CGPointMake(0, headerOffsetY);
...
- (OSSVAccountsHeadView *)tableHeaderView {
    if (!_tableHeaderView) {
        CGRect rect = CGRectMake(0, 0, SCREEN_WIDTH, [OSSVAccountsHeadView accountHeaderContentH:NO]);
        _tableHeaderView = [[OSSVAccountsHeadView alloc] initWithFrame:rect];
...
- (OSSVAccountTablesMenuView *)menuView {
    if (!_menuView) {
        CGRect rect = CGRectMake(0, 0, SCREEN_WIDTH, kSelectMenuViewHeight);
        _menuView = [[OSSVAccountTablesMenuView alloc] initWithFrame:rect];
        _menuView.backgroundColor = [OSSVThemesColors col_F5F5F5];
        if (APP_TYPE == 3) {
            _menuView.backgroundColor = [OSSVThemesColors stlWhiteColor];
...
- (STLAccountTableView *)tableView {
    ...
    if (!_tableView) {
        ...
        /** 注册页面用到的所有Cell类型 */
        NSArray *cellTypeInfoArr = [OSSVAccountHeadCellTypeModel fetchAllCellTypeArray];
        ...
        [_tableView registerClass:[OSSVAccountHeadBaseCell class] forCellReuseIdentifier:NSStringFromClass([OSSVAccountHeadBaseCell class])];
    }
    return _tableView;
}

- (OSSVAccountsTableFootView *)tableFooterView {
    if (!_tableFooterView) {
        ...
        @weakify(self);
        _tableFooterView = [[OSSVAccountsTableFootView alloc] initWithFrame:rect selectedGoodsBlock:^(id  _Nonnull goodsModel, AccountGoodsListType dataType, NSInteger index,NSString *requestId) {
        ...
        _tableFooterView.backgroundColor = [OSSVThemesColors col_F5F5F5];
