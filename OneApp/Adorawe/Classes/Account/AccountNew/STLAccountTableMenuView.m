...
- (UIColor *)menuView:(WMMenuView *)menu titleColorForState:(WMMenuItemState)state atIndex:(NSInteger)index {
    if (state == WMMenuItemStateSelected) {
        return [STLThemeColor col_0D0D0D];
    }
    return [STLThemeColor col_6C6C6C];
}
...
//        _menuView.fontName = @"ArialHebrew-Bold";
        ...
        _menuView.lineColor = [STLThemeColor col_0D0D0D];
...
- (UIView *)underLineView {
    if (!_underLineView) {
        ...
        _underLineView.backgroundColor = [STLThemeColor col_0D0D0D];
        if (APP_TYPE == 3) {
            _underLineView.backgroundColor = [STLThemeColor col_E1E1E1];
        }
