...
@property (nonatomic, strong) OSSVWhatseAppeContentView     *whatsAppView; //whatsAPP 背景视图
@property (nonatomic, strong) OSSVAccounteMyeOrderseViewModel   *viewModel;
...
- (void)initView {
    ...
    //v 2.0.0 隐藏
    OSSVAccountsManager.sharedManager.sysIniModel.isShowWhatsAppSubscribe = NO;
    //是否展示WhatsApp 引流视图
    if(OSSVAccountsManager.sharedManager.sysIniModel.isShowWhatsAppSubscribe) {
...
- (OSSVWhatseAppeContentView *)whatsAppView {
    if (!_whatsAppView) {
        _whatsAppView = [[OSSVWhatseAppeContentView alloc] init];
...
- (UIView *)ipChangedView{
    ...
    if (!_ipChangedView) {
        ...
        _ipChangedView.backgroundColor = OSSVThemesColors.col_FBE9E9;
        _ipChangedView.hidden = YES;
        UILabel *contentLbl = [[UILabel alloc] init];
        contentLbl.numberOfLines = 0;
        contentLbl.font = [UIFont systemFontOfSize:12];
        contentLbl.textColor = OSSVThemesColors.col_B62B21;
...
-(void)customerServiceBtnAction {
    OSSVServiceeTipseAlertView *popView = [[OSSVServiceeTipseAlertView alloc] init];
    [popView show];
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[OSSVAccounteMyOrderseCell class] forCellReuseIdentifier:NSStringFromClass(OSSVAccounteMyOrderseCell.class)];
...
- (OSSVAccounteMyeOrderseViewModel *)viewModel {
    if (!_viewModel) {
        _viewModel = [[OSSVAccounteMyeOrderseViewModel alloc] init];
