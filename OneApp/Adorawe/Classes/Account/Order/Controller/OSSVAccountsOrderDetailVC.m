...
#import "YYText.h"
#import "OSSVOrdereTrackeeView.h" //新增物流信息
#import "OSSVTransporteSpliteView.h" //拆单物流信息
...
/******************************** Models数据模型 *********************************/
#import "OSSVAccounteOrderseDetailShippingeModel.h"//物流信息数据模型
#import "OSSVAccounteMyOrderseDetailModel.h"//订单详情页数据模型
#import "OSSVAddresseBookeModel.h"//地址数据模型
/*支付数据*/
...
/*取消,支付按钮容器*/
@property (nonatomic,strong) OSSVBottomeButtonseView *payBottomView;
...
/*地址信息*/
@property (nonatomic,strong) OSSVCartOrderInformationAddressView *addressView;
/*仓库title*/
@property (nonatomic,strong) UILabel                        *wareHouseTitleLabel;
/*商品信息*/
@property (nonatomic,strong) OSSVAccounteOrdereDetaileView         *goodsView;

/*订单号*/
@property (nonatomic,strong) OSSVOrdereDetaileTitileDesceView   *orderNoView;
/*订单时间*/
@property (nonatomic,strong) OSSVOrdereDetaileTitileDesceView   *orderDateView;
/*物流方式*/
@property (nonatomic,strong) OSSVOrdereDetaileTitileDesceView   *orderShipMethodView;
/*支付方式*/
@property (nonatomic,strong) OSSVOrdereDetaileTitileDesceView   *orderPaymentMethodView;


/*折扣*/
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalSavePriceCostView;

/********************商品价格,邮费,优惠券等价格*********************/
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalProdcetCostView;
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalShippingCostView;
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalTrackingCostView;
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalInsuranceCostView;
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalCouponCostView;
///支付方式优惠金额
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalDiscountCostView;
///金币金额
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalCoinDiscountCostView;
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalCodCostView;
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalPointCostView;
///部分付款的层
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalPartyPayCostView;
///<COD商品订单总额 总额价格
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalCodOrderCostView;
///<COD商品取整类型 取整数额
@property (nonatomic, strong) OSSVOrdeerDetaileTitlePriceeView *totalCodOrderDistypeCostView;
...
@property (nonatomic, strong) OSSVOrderseTimeeDownView *countDownView;
/*========================================分割线======================================*/

/*订单详情页数据模型*/
@property (nonatomic,strong) OSSVAccounteMyOrderseDetailModel *orderDetailModel;
/*当前VC的ViewModel*/
...
@property (nonatomic, strong) OSSVAccounteMyeOrderseViewModel              *myOrderlistModel;
...
- (UIView *)topOrderStateView {
    if (!_topOrderStateView) {
        ...
        _topOrderStateView.backgroundColor = [OSSVThemesColors col_60CD8E];
...
- (UILabel *)statusLabel {
    if (!_statusLabel) {
        ...
        statusLabel.textColor = [OSSVThemesColors stlWhiteColor];
...
- (UIView *)trackingBottomView {
    if (!_trackingBottomView) {
        ...
        trackingBottomView.backgroundColor = OSSVThemesColors.col_FFFFFF;
...
- (OSSVCartOrderInformationAddressView *)addressView {
    if (!_addressView) {
        OSSVCartOrderInformationAddressView *addressView = [OSSVCartOrderInformationAddressView new];
...
- (UIView *)shippingBottomView {
    if (!_shippingBottomView) {
        ...
        shippingBottomView.backgroundColor = OSSVThemesColors.col_FFFFFF;
...
//        shipMethodLabel.textColor = OSSVThemesColors.col_333333;
//        _shipMethodLabel = shipMethodLabel;
//    }
//    return _shipMethodLabel;
//}

- (OSSVOrdereDetaileTitileDesceView *)orderShipMethodView {
    if (!_orderShipMethodView) {
        _orderShipMethodView = [[OSSVOrdereDetaileTitileDesceView alloc] initWithFrame:CGRectZero title:STLLocalizedString_(@"shipMethod", nil)];
...
//        paymentMethodLabel.textColor = OSSVThemesColors.col_333333;
//        _paymentMethodLabel = paymentMethodLabel;
//    }
//    return _paymentMethodLabel;
//}
- (OSSVOrdereDetaileTitileDesceView *)orderPaymentMethodView {
    if (!_orderPaymentMethodView) {
        _orderPaymentMethodView = [[OSSVOrdereDetaileTitileDesceView alloc] initWithFrame:CGRectZero title:STLLocalizedString_(@"paymentMethod", nil)];
    }
...
//        orderNoLabel.textColor = OSSVThemesColors.col_333333;
//        _orderNoLabel = orderNoLabel;
//    }
//    return _orderNoLabel;
//}

- (OSSVOrdereDetaileTitileDesceView *)orderNoView {
    if (!_orderNoView) {
        _orderNoView = [[OSSVOrdereDetaileTitileDesceView alloc] initWithFrame:CGRectZero title:STLLocalizedString_(@"orderNo", nil)];
...
//        orderDate.textColor = OSSVThemesColors.col_333333;
//        _orderDate = orderDate;
//    }
//    return _orderDate;
//}

- (OSSVOrdereDetaileTitileDesceView *)orderDateView {
    if (!_orderDateView) {
        _orderDateView = [[OSSVOrdereDetaileTitileDesceView alloc] initWithFrame:CGRectZero title:STLLocalizedString_(@"orderDate", nil)];
    }
...
- (UIView *)goodsBottomView {
    if (!_goodsBottomView) {
        ...
        _goodsBottomView.backgroundColor = [OSSVThemesColors stlWhiteColor];
...
- (UILabel *)wareHouseTitleLabel {
    if (!_wareHouseTitleLabel) {
        ...
        _wareHouseTitleLabel.textColor = [OSSVThemesColors col_0D0D0D];
...
- (UIView *)totalBottomView {
    if (!_totalBottomView) {
        ...
        totalBottomView.backgroundColor = OSSVThemesColors.col_FFFFFF;
...
- (OSSVOrdeerDetaileTitlePriceeView *)totalProdcetCostView {
    if (!_totalProdcetCostView) {
        _totalProdcetCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"protudts_total",nil)]];
    }
    return _totalProdcetCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalShippingCostView {
    if (!_totalShippingCostView) {
        _totalShippingCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"shipCost",nil)]];
    }
    return _totalShippingCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalDiscountCostView {
    if (!_totalDiscountCostView) {
        _totalDiscountCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"disCountTitle",nil)]];
        _totalDiscountCostView.priceLabel.textColor = [OSSVThemesColors col_B62B21];

    }
    return _totalDiscountCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalTrackingCostView {
    if (!_totalTrackingCostView) {
        _totalTrackingCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"tracking",nil)]];
    }
    return _totalTrackingCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalInsuranceCostView {
    if (!_totalInsuranceCostView) {
        _totalInsuranceCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"insurance",nil)]];
    }
    return _totalInsuranceCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalCodCostView {
    if (!_totalCodCostView) {
        _totalCodCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"codCost",nil)]];
    }
    return _totalCodCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalSavePriceCostView {
    if (!_totalSavePriceCostView) {
        _totalSavePriceCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"save",nil)]];
        _totalSavePriceCostView.priceLabel.textColor = [OSSVThemesColors col_B62B21];

    }
    return _totalSavePriceCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalCoinDiscountCostView {
    if (!_totalCoinDiscountCostView) {
        _totalCoinDiscountCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"CoinsCost",nil)]];
        _totalCoinDiscountCostView.priceLabel.textColor = [OSSVThemesColors col_B62B21];
    }
    return _totalCoinDiscountCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalCouponCostView {
    if (!_totalCouponCostView) {
        _totalCouponCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"orderInfo_coupon",nil)]];
        _totalCouponCostView.priceLabel.textColor = [OSSVThemesColors col_B62B21];

    }
    return _totalCouponCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalPointCostView {
    if (!_totalPointCostView) {
        _totalPointCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"orderInfo_point",nil)]];
        _totalPointCostView.priceLabel.textColor = [OSSVThemesColors col_B62B21];
    }
    return _totalPointCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalPartyPayCostView {
    if (!_totalPartyPayCostView) {
        _totalPartyPayCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"AmountPaid",nil)]];
    }
    return _totalPartyPayCostView;
}

- (OSSVOrdeerDetaileTitlePriceeView *)totalCodOrderCostView {
    if (!_totalCodOrderCostView) {
        _totalCodOrderCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"total",nil)]];
    }
    return _totalCodOrderCostView;
}


- (OSSVOrdeerDetaileTitlePriceeView *)totalCodOrderDistypeCostView {
    if (!_totalCodOrderDistypeCostView) {
        _totalCodOrderDistypeCostView = [[OSSVOrdeerDetaileTitlePriceeView alloc] initWithFrame:CGRectZero title:[NSString stringWithFormat:@"%@ :",STLLocalizedString_(@"COD_discount",nil)]];
        _totalCodOrderDistypeCostView.priceLabel.textColor = [OSSVThemesColors col_B62B21];
    }
...
- (UIView *)totalLineView {
    if (!_totalLineView) {
        ...
        _totalLineView.backgroundColor = [OSSVThemesColors col_EEEEEE];
    }
...
- (UILabel *)reallyPayTotalLabel {
    if (!_reallyPayTotalLabel) {
        ...
        _reallyPayTotalLabel.textColor = [OSSVThemesColors col_0D0D0D];
...
- (UILabel *)reallyTotapPayPriceLabel {
    if (!_reallyTotapPayPriceLabel) {
        ...
        _reallyTotapPayPriceLabel.textColor = [OSSVThemesColors col_0D0D0D];
    }
...
- (UILabel *)reallyTaxLabel {
    if (!_reallyTaxLabel) {
        ...
        _reallyTaxLabel.textColor = [OSSVThemesColors col_B2B2B2];
    }
...
- (UIView *)bottomSafeView {
    if (!_bottomSafeView) {
        ...
        _bottomSafeView.backgroundColor = [OSSVThemesColors stlWhiteColor];
    }
...
- (void) initView {
    ...
    VirtualView.backgroundColor = OSSVThemesColors.col_FFFFFF;
...
- (void)clickPayNow:(UIButton *)sender {
    ...
    [self.viewModel requestPayNowOrder:self.orderDetailModel.orderId completion:^(OSSVOrdereInforeModel *obj) {
        if (obj) {
...
- (void)buyAgainOrderRefresh:(BOOL)refresh {
    ...
    OSSVOrdereBuyeAgainAip *buyAgainApi = [[OSSVOrdereBuyeAgainAip alloc] initWithOrderId:STLToString(self.orderId)];
    ...
    [buyAgainApi startWithBlockSuccess:^(__kindof OSSVBasesRequests *request) {
        ...
        [OSSVAnalyticsTool analyticsSensorsEventWithName:@"AddToCartOrder" parameters:@{@"is_success":@(state),@"order_sn":STLToString(self.orderDetailModel.orderSn)}];
...
- (void)analyticsAddToCartIsSuccess:(BOOL)isSuccess{
    ...
    [self.orderDetailModel.goodsList enumerateObjectsUsingBlock:^(OSSVAccounteOrdersDetaileGoodsModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)clickReview:(UIButton *)sender {
    ...
    OSSVOrdersReviewVC *orderReviewCtrl = [[OSSVOrdersReviewVC alloc] init];
...
- (void)reloadData {
    /*请求订单数据*/
    @weakify(self);
    [self.viewModel requestNetwork:self.orderId completion:^(OSSVAccounteMyOrderseDetailModel *obj) {
        /*赋值外部变量->数据*/
        @strongify(self);
...
- (void)handleOrdersDetailModel:(OSSVAccounteMyOrderseDetailModel *)obj {
    if (!self) {return;}
    if (!obj) {
        return;
    }
    ...
    self.topOrderStateView.backgroundColor = [OSSVThemesColors col_60CD8E];
    [self.countDownView updateTipsTypeMsg:STLLocalizedString_(@"Remaining_payment_time", nil)];
    
    [self.payBottomView.buyAgainBtn setBackgroundColor:[OSSVThemesColors stlClearColor]];
    [self.payBottomView.buyAgainBtn setTitleColor:[OSSVThemesColors col_0D0D0D] forState:UIControlStateNormal];
    ...
    if ([obj.payCode isEqualToString:@"Cod"]) {
        ...
        if (obj.orderStatus == OrderStateTypeWaitingForPayment || obj.orderStatus == OrderStateTypeWaitConfirm) {
            ...
            self.topOrderStateView.backgroundColor = [OSSVThemesColors col_CCCCCC];
    ...
    if (self.orderDetailModel.orderStatus == OrderStateTypeWaitingForPayment
        ...
        self.topOrderStateView.backgroundColor = [OSSVThemesColors col_B62B21];
...
//    self.orderNoLabel.text = [NSString stringWithFormat:@"%@ : %@",STLLocalizedString_(@"orderNo",nil),obj.orderSn];
    ...
    OSSVAddresseBookeModel *bookModel = [[OSSVAddresseBookeModel alloc] init];
...
//    }
    ...
    if (payBottomViewH > 0) {
        ...
        if (hasBuyAgain && !hasPay) {
            [self.payBottomView.buyAgainBtn setBackgroundColor:[OSSVThemesColors col_0D0D0D]];
            [self.payBottomView.buyAgainBtn setTitleColor:[OSSVThemesColors stlWhiteColor] forState:UIControlStateNormal];
        }
...
#pragma mark ------处理 物流信息
- (void)handleTrackInfor:(OSSVAccounteMyOrderseDetailModel *)obj {
...
//    if (obj.shipping.waitingShip) {
    ...
    ///occ
//    OSSVTrackincWaitingcShipMode *moddd = [[OSSVTrackincWaitingcShipMode alloc] init];
...
//    obj.is_split = @"0";
//
//    OSSVTransportineeModel *ttModel = [[OSSVTransportineeModel alloc] init];
...
//    obj.shipping.transport = ttModel;
        ...
        if ([STLToString(obj.is_split) isEqualToString:@"1"]) {
            ...
            OSSVTransporteSpliteView *splitView = [[OSSVTransporteSpliteView alloc] init];
            splitView.backgroundColor = OSSVThemesColors.col_FFFFFF;
        ...
        if (obj.shipping.waitingShip) {
            @weakify(self)
            //新增物流信息
            OSSVOrdereTrackeeView *trackView = [[OSSVOrdereTrackeeView alloc] init];
            trackView.backgroundColor = OSSVThemesColors.col_FFFFFF;
            ...
            if (obj.shipping.transport) {
                ...
                if (STLJudgeNSArray(obj.shipping.transport.trackArray)) {
                    ...
                    if (trackArray.count) {
                        //取第一条最新的动态
                        OSSVTransporteTrackeMode *model = trackArray[0];
...
#pragma mark 处理 商品信息
- (void)handleGoodsImgae:(OSSVAccounteMyOrderseDetailModel *)obj {
    
    if (obj.goodsList && [obj.goodsList count]) {
        //仓库名字
        OSSVAccounteOrdersDetaileGoodsModel *detailModel= obj.goodsList[0];
        self.wareHouseTitleLabel.text = detailModel.wareHouseName;
        
        /*数量为单件的时候布局*/
        if (obj.goodsList.count == 1) {
            self.goodsView = [OSSVAccounteOrdereDetaileView new];
            ...
            [obj.goodsList enumerateObjectsUsingBlock:^(OSSVAccounteMyOrderseDetailModel * _Nonnull goodsDetail, NSUInteger idx, BOOL * _Nonnull stop) {
                self.goodsView = [OSSVAccounteOrdereDetaileView new];
                self.goodsView.delegate = self;
...
-(void)showTrackTips:(NSString *)tipString
{
    ...
    tips.text = tipString;
    tips.textColor = [OSSVThemesColors col_0D0D0D];
...
- (void)actionCopyOrderNumber {
    ...
    [OSSVAnalyticsTool analyticsGAEventWithName:@"copy_orderNo" parameters:@{
           @"screen_group":@"OrderDetail",
           @"action":@"Copy"}];
}
...
//            label.text = @"";
//            label.font = [UIFont systemFontOfSize:12];
//            label.textColor = [OSSVThemesColors stlBlackColor];
//            label;
//        });
//    }
...
-(OSSVOrderseTimeeDownView *)countDownView
{
    if (!_countDownView) {
        _countDownView = [[OSSVOrderseTimeeDownView alloc] init];
...
- (OSSVBottomeButtonseView *)payBottomView {
    if (!_payBottomView) {
        _payBottomView = [[OSSVBottomeButtonseView alloc] initWithFrame:CGRectZero];
        _payBottomView.backgroundColor = [OSSVThemesColors stlWhiteColor];
...
- (UIView *)waitCheckView {
    if (!_waitCheckView) {
        ...
        _waitCheckView.backgroundColor = [OSSVThemesColors stlWhiteColor];
...
- (UILabel *)codCheckTipLabel {
    if (!_codCheckTipLabel) {
        ...
        _codCheckTipLabel.textColor = [OSSVThemesColors col_0D0D0D];
...
- (void)paySuccesWithProductCod:(BOOL)state {
    ...
    for (NSInteger i = 0; i < self.orderDetailModel.goodsList.count; i ++) {
        OSSVAccounteOrdersDetaileGoodsModel *goodsModel = (OSSVAccounteOrdersDetaileGoodsModel*)self.orderDetailModel.goodsList[i];
...
- (OSSVAccounteMyeOrderseViewModel *)myOrderlistModel {
    if (!_myOrderlistModel) {
        _myOrderlistModel = [[OSSVAccounteMyeOrderseViewModel alloc] init];
    }
