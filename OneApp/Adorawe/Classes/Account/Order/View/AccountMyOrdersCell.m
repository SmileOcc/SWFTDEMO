...
#import "AccountMyOrdersListModel.h"
#import "STLLableEdgeInsets.h"
#import "OSSVPaymenteTipseAlertView.h"
#import "AccountMyOrdersGoodsModel.h"
#import "STLOrderItemButtonView.h"
#import "STLOrderGoodsView.h"
...
@property (nonatomic,strong) STLOrderGoodsView  *orderGoodsView;
...
@property (nonatomic,strong) STLOrderRepurchaseTipView *repurchaseTipView;

@property (nonatomic,strong) STLOrderItemButtonView           *cancleBtn;//取消按钮
@property (nonatomic,strong) STLOrderItemButtonView           *payBtn;//支付按钮
@property (nonatomic,strong) STLOrderItemButtonView           *buyAgainBtn;//再次购买
@property (nonatomic,strong) STLOrderItemButtonView           *reviewBtn;//评论
@property (nonatomic,strong) STLOrderItemButtonView           *shipmentBtn;//物流
@property (nonatomic,strong) STLOrderItemButtonView           *tempPayBtn;//支付按钮
...
- (void)initView {
    ...
    ws.backgroundColor = [STLThemeColor col_F5F5F5];
...
//    [paypopView show];
    ...
    parStyle.lineSpacing = [STLAlertViewNew lineSpace];
    ...
    [STLAlertViewNew showAlertWithFrame:[UIScreen mainScreen].bounds alertType:STLAlertTypeButtonColumn isVertical:YES messageAlignment:NSTextAlignmentCenter isAr:NO showHeightIndex:0 title:@"" message:attString buttonTitles:@[STLLocalizedString_(@"yes", nil)] buttonBlock:^(NSInteger index , NSString * _Nonnull title) {
...
- (void)setOrderListModel:(AccountMyOrdersListModel *)orderListModel {
    ...
    [self.buyAgainBtn.button setBackgroundColor:[STLThemeColor stlClearColor]];
    [self.buyAgainBtn.button setTitleColor:[STLThemeColor col_0D0D0D] forState:UIControlStateNormal];
    ...
    //状态太多，默认绿色
    self.colorStatue.backgroundColor = [STLThemeColor col_60CD8E];
...
//    }];
    ...
    self.orderTimeLabel.text = [STLCommonManager dateFormatString:[STLCommonManager sharedManager].arDateTimeFormatter time:STLToString(_orderListModel.add_time)];
...
//        self.waitCheckView.backgroundColor = STLThemeColor.col_FFF5DF;
    ...
    if ([orderListModel.orderStatus integerValue] == OrderStateTypeCancelled
        ...
        self.colorStatue.backgroundColor = [STLThemeColor col_CCCCCC];
//        self.waitCheckView.backgroundColor = [STLThemeColor col_FAFAFA];
        ...
        self.colorStatue.backgroundColor = [STLThemeColor col_B62B21];
        if (![orderListModel.payCode isEqualToString:@"Cod"]) {
            ...
            if ([orderListModel.orderStatus integerValue] == OrderStateTypeWaitingForPayment
                ...
                [self.payBtn.button setBackgroundColor:[STLThemeColor col_0D0D0D]];
                [self.payBtn.button setTitleColor:[STLThemeColor stlWhiteColor] forState:UIControlStateNormal];
...
//    self.totalPrice.text = [NSString stringWithFormat:@"%@ : %@",STLLocalizedString_(@"total",nil),orderListModel.money_info.payable_amount_converted_symbol];
    ...
    if (bottomHeight > 0) {
        ...
        if (hasBuyAgain && !hasPay) {
            [self.buyAgainBtn.button setBackgroundColor:[STLThemeColor col_0D0D0D]];
            [self.buyAgainBtn.button setTitleColor:[STLThemeColor stlWhiteColor] forState:UIControlStateNormal];
        }
    }
...
#pragma mark - 商品图片
- (void)createGoodsImgView:(AccountMyOrdersListModel *)orderListModel {
...
- (UIView *)bgView {
    if (!_bgView) {
        ...
        _bgView.backgroundColor = [STLThemeColor stlWhiteColor];
...
- (UIView *)lineView {
    if (!_lineView) {
        ...
        _lineView.backgroundColor = [STLThemeColor col_EEEEEE];
    }
    return _lineView;
}
...
- (UILabel *)statusLabel {
    if (!_statusLabel) {
        ...
        _statusLabel.textColor = [STLThemeColor col_0D0D0D];
...
- (UILabel *)orderTimeLabel {
    if (!_orderTimeLabel) {
        ...
        _orderTimeLabel.textColor = [STLThemeColor col_B2B2B2];
...
- (UIView *)middleView {
    if (!_middleView) {
        ...
        _middleView.backgroundColor = [STLThemeColor stlWhiteColor];
    }
    return _middleView;
}

- (STLOrderGoodsView *)orderGoodsView {
    if (!_orderGoodsView) {
        _orderGoodsView = [[STLOrderGoodsView alloc] init];
    }
...
- (UIView *)totalView {
    if (!_totalView) {
        ...
        _totalView.backgroundColor = [STLThemeColor stlWhiteColor];
    }
    return _totalView;
}
...
- (UIView *)secondLineView {
    if (!_secondLineView) {
        ...
        _secondLineView.backgroundColor = [STLThemeColor col_EEEEEE];
    }
...
- (UIView *)threeLineView {
    if (!_threeLineView) {
        ...
        _threeLineView.backgroundColor = [STLThemeColor col_EEEEEE];
    }
...
- (UILabel *)totalPrice {
    if (!_totalPrice) {
        ...
        _totalPrice.textColor = [STLThemeColor col_0D0D0D];
...
- (UILabel *)totalPriceLbl {
    if (!_totalPriceLbl) {
        ...
        _totalPriceLbl.textColor = [STLThemeColor col_0D0D0D];
...
- (UILabel *)totalItemsLabel {
    if (!_totalItemsLabel) {
        ...
        _totalItemsLabel.textColor = [STLThemeColor col_666666];
...
- (UILabel *)taxLabel {
    if (!_taxLabel) {
        ...
        _taxLabel.textColor = [STLThemeColor col_B2B2B2];
...
- (STLOrderItemButtonView *)cancleBtn {
    if (!_cancleBtn) {
        @weakify(self)
        _cancleBtn = [[STLOrderItemButtonView alloc] initWithFrame:CGRectZero title:APP_TYPE ? STLLocalizedString_(@"cancel",nil): [STLLocalizedString_(@"cancel",nil) uppercaseString] tapBlock:^{
            @strongify(self)
...
- (STLOrderItemButtonView *)payBtn {
    if (!_payBtn) {
        @weakify(self)
        _payBtn = [[STLOrderItemButtonView alloc] initWithFrame:CGRectZero title:[STLLocalizedString_(@"payNow",nil) uppercaseString] tapBlock:^{
            @strongify(self)
...
- (STLOrderItemButtonView *)tempPayBtn {
    if (!_tempPayBtn) {
        _tempPayBtn = [[STLOrderItemButtonView alloc] initWithFrame:CGRectZero title:[STLLocalizedString_(@"payNow",nil) uppercaseString] tapBlock:^{
        }];
...
- (STLOrderItemButtonView *)buyAgainBtn {
    if (!_buyAgainBtn) {
        @weakify(self)
        _buyAgainBtn = [[STLOrderItemButtonView alloc] initWithFrame:CGRectZero title:[STLLocalizedString_(@"Repurchase",nil) uppercaseString] tapBlock:^{
            @strongify(self)
...
//物流查询button
- (STLOrderItemButtonView *)shipmentBtn {
    if (!_shipmentBtn) {
        @weakify(self)
        
        _shipmentBtn = [[STLOrderItemButtonView alloc] initWithFrame:CGRectZero title:APP_TYPE == 3 ? STLLocalizedString_(@"SHIPMENT",nil) : [STLLocalizedString_(@"SHIPMENT",nil) uppercaseString] tapBlock:^{
            @strongify(self)
...
//
- (STLOrderItemButtonView *)reviewBtn {
    if (!_reviewBtn) {
        @weakify(self)
        _reviewBtn = [[STLOrderItemButtonView alloc] initWithFrame:CGRectZero title:[STLLocalizedString_(@"reviews",nil) uppercaseString] tapBlock:^{
            @strongify(self)
...
- (STLLableEdgeInsets *)codOrderStatueLabel {
    if (!_codOrderStatueLabel) {
        ...
        _codOrderStatueLabel.textColor = STLThemeColor.col_FF9900;
        _codOrderStatueLabel.backgroundColor = STLThemeColor.col_FFFFFF;
...
-(UILabel *)countDownTipsLabel
{
    if (!_countDownTipsLabel) {
        _countDownTipsLabel = ({
            ...
            label.textColor = [STLThemeColor col_6C6C6C];
            label.font = [UIFont systemFontOfSize:12];
            label.backgroundColor = [STLThemeColor stlClearColor];
            //v1.4.6显示位置刚好相反
...
-(ZJJTimeCountDownLabel *)countDownLabel
{
    if (!_countDownLabel) {
        _countDownLabel = ({
            ...
            //字体颜色
            label.textColor = [STLThemeColor col_B62B21];
            label.textHeight = 18;
            //单个文本背景颜色
...
- (UIView *)waitCheckBgView {
    if (!_waitCheckBgView) {
        ...
        _waitCheckBgView.backgroundColor = [STLThemeColor stlWhiteColor];
    }
...
- (UIView *)waitCheckView {
    if (!_waitCheckView) {
        ...
        _waitCheckView.backgroundColor = STLThemeColor.col_FFF3E4;
    }
...
- (UILabel *)codCheckTipLabel {
    if (!_codCheckTipLabel) {
        ...
        _codCheckTipLabel.textColor = [STLThemeColor col_6C6C6C];
...
- (STLOrderRepurchaseTipView *)repurchaseTipView {
    if (!_repurchaseTipView) {
        _repurchaseTipView = [[STLOrderRepurchaseTipView alloc] initWithRect:CGRectZero title:@""];
