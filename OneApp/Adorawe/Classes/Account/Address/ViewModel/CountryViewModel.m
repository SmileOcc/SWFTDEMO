...
#import "AddressCountryHeaderView.h"
#import "CountryApi.h"
#import "STLCountryViewCell.h"
...
//            [AccountManager sharedManager].countryList = result;
//            if (completion) {
//                completion(result);
//            }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    // 这里实际上是进行 复用了哦
    AddressCountryHeaderView *headerView = [AddressCountryHeaderView addressCountryHeaderViewWithTableView:tableView section:section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    STLCountryViewCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass(STLCountryViewCell.class)];
...
//    [cell.imageView yy_setImageWithURL:[NSURL URLWithString:model.picture] placeholder:[UIImage imageNamed:@"placeholder_pdf"] options:YYWebImageOptionProgressiveBlur | YYWebImageOptionSetImageWithFadeAnimation completion:nil];
    ...
    cell.textLabel.textColor = STLThemeColor.col_333333;
    if ([self.countryName isEqualToString:model.countryName]) {
        cell.textLabel.textColor = STLThemeColor.col_FF9522;
    } else {
        cell.textLabel.textColor = STLThemeColor.col_333333;
    }
    return cell;
...
//    for (CountryListModel *obj in [AccountManager sharedManager].countryList) {
