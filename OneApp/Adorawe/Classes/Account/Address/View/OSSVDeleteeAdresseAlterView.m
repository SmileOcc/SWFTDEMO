...
@interface OSSVDeleteeAdresseAlterView ()
...
@implementation OSSVDeleteeAdresseAlterView
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.text = STLLocalizedString_(@"deleteAddress", nil);
    }
    return _titleLabel;
}
...
- (UIButton *)cancelButton {
    if (!_cancelButton) {
        ...
        if (APP_TYPE == 3) {
            [_cancelButton setTitle:STLLocalizedString_(@"no", nil) forState:UIControlStateNormal];
        } else {
            [_cancelButton setTitle:STLLocalizedString_(@"no", nil).uppercaseString forState:UIControlStateNormal];
        }
...
- (UIButton *)sureButton {
    if (!_sureButton) {
        ...
        _sureButton.backgroundColor = OSSVThemesColors.col_0D0D0D;
        [_sureButton setTitle:STLLocalizedString_(@"yes", nil).uppercaseString forState:UIControlStateNormal];
...
- (void)layoutSubviews {
    ...
    [self.contentView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(134*kScale_375);
    }];
...
- (void)sureAction {
    if (_delegate && [_delegate respondsToSelector:@selector(makeSureAction)]) {
        [self.delegate makeSureAction];
    }
}
@end
