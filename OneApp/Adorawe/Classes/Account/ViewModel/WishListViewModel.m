...
- (instancetype)init {
    self = [super init];
    if (self) {
        [[STLAnalyticsInjectManager shareInstance] analyticsInject:self injectObject:self.wishListAnalyticsManager];
    }
    return self;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [STLAnalytics analyticsGAEventWithName:@"view_product_detail" parameters:@{
    ...
    NSDictionary *dic = @{kAnalyticsAction:[STLAnalytics sensorsSourceStringWithType:STLAppsflyerGoodsSourceWishlist sourceID:@""],
                          kAnalyticsUrl:@"",
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    cornerView.backgroundColor = [STLThemeColor stlWhiteColor];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    footerView.backgroundColor = [STLThemeColor stlClearColor];
    if (APP_TYPE == 3) {
        return footerView;
    }
...
-(void)accountMyWishCell:(OSSVWishListsTableCell *)cell addToCart:(OSSVAccountMysWishsModel *)wishModel{
    ...
    [STLAnalytics analyticsGAEventWithName:@"item_pop_ups" parameters:@{@"screen_group" : @"WishList",
                                                                        @"position"     : @"WishList",
...
- (void)accountMyWishCell:(OSSVWishListsTableCell *)cell deleteModel:(OSSVAccountMysWishsModel *)wishModel {
    ...
    [STLAnalytics analyticsGAEventWithName:@"wishList_action" parameters:@{
           @"screen_group":@"WishList",
    ...
    [STLAlertViewNew showAlertWithAlertType:STLAlertTypeButton isVertical:YES messageAlignment:NSTextAlignmentCenter isAr:NO showHeightIndex:1 title:@"" message:STLLocalizedString_(@"removeItems", nil) buttonTitles:APP_TYPE == 3 ? lowserTitle : upperTitle buttonBlock:^(NSInteger index, NSString * _Nonnull title) {
        if (index==1) {
