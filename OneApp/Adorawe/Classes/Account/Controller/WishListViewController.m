...
- (void)initSubViews {
    ...
    self.tableView.backgroundColor = STLThemeColor.col_F5F5F5;
    ...
    self.tableView.blankPageViewActionBlcok = ^(STLBlankPageViewStatus status) {
        @strongify(self)
        STLAdvEventModel *model = [[STLAdvEventModel alloc] init];
        model.actionType = 0;
        [STLAdvEventManager advEventTarget:self withEventModel:model];
...
- (void)actionCart {
    STLCartViewCtrl *cartVC = nil;
    NSMutableArray *arr = [NSMutableArray arrayWithArray:self.navigationController.viewControllers];
    NSInteger index = 0;
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:STLCartViewCtrl.class] && index != 0) {
            cartVC = (STLCartViewCtrl*)vc;
            break;
        }
        index++;
    }
    ...
    if (!cartVC) {
        cartVC = [[STLCartViewCtrl alloc] init];
    }
