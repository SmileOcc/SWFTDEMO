...
- (void)requestNetwork:(id)parmaters completion:(void (^)(id obj))completion failure:(void (^)(id obj))failure {
    ...
    [[STLNetworkStateManager sharedManager] networkState:^{
        @strongify(self)
        @weakify(self)
        OSSVTrackingeInformationeAip *trackingeInformationeAip = [[OSSVTrackingeInformationeAip alloc] initWithTrackVal:trackVal trackType:trackType];
        [trackingeInformationeAip.accessoryArray addObject:[[STLRequestAccessory alloc] initWithApperOnView:self.controller.view]];
        [trackingeInformationeAip  startWithBlockSuccess:^(__kindof OSSVBasesRequests *request) {
            @strongify(self)
            id requestJSON = [OSSVNSStringTool desEncrypt:request];
            self.dataArray = [self dataAnalysisFromJson: requestJSON request:trackingeInformationeAip];
            if (completion) {
