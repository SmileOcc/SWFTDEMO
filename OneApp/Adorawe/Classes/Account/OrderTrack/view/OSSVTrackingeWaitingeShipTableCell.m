//
//  OSSVTrackingeWaitingeShipTableCell.m
// XStarlinkProject
//
...
@interface OSSVTrackingeWaitingeShipTableCell ()
@property (nonatomic, strong) UIView              *shipBgView;         //待发货背景
@property (nonatomic, strong) YYAnimatedImageView *shipImgView;        //待发货图片
...
@implementation OSSVTrackingeWaitingeShipTableCell
...
- (void)setModel:(OSSVTrackingcTotalInformcnModel *)model {
    _model = model;
    self.titleLabel.text = STLToString(model.waitingShip.logistics_status);
    self.subTitleLabel.text = STLToString(model.waitingShip.desc);
    NSString *dateStr = STLToString(model.waitingShip.date);
    if (dateStr.length) {
    ...
    if (model.alreadySign) {
        ...
        self.shipImgView.image = [UIImage imageNamed:@"wait_ship_gray"];
...
- (YYAnimatedImageView *)shipImgView {
    if (!_shipImgView) {
        ...
        _shipImgView.image = [UIImage imageNamed:@"wait_ship_white"];
    }
    return _shipImgView;
}
...
//        _titleLabel.text = @"Waiting for shipment";
    }
    return _titleLabel;
}
...
//        _subTitleLabel.text = @"Your order is under Preparing";
    }
