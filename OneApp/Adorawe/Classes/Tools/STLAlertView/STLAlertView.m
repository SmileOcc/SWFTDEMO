...
#define  OKFONTDEFAULT_BUTTON(fontSize)                ([UIFont stl_buttonFont:fontSize])
...
//按钮普通状态字体颜色
#ifndef  OKAlertView_BtnTitleFormateNorColor
#define  OKAlertView_BtnTitleFormateNorColor           OKColorFromHex(0x666666)
#endif

//按钮普通状态字体颜色
#ifndef  OKAlertView_BtnTitleFormateHighColor
#define  OKAlertView_BtnTitleFormateHighColor           OKColorFromHex(0xFFFFFF)
#endif

//按钮颜色
#ifndef  OKAlertView_BtnBgFormateHighColor
#define  OKAlertView_BtnBgFormateHighColor             [OSSVThemesColors col_0D0D0D]
#endif

//按钮边框颜色
#ifndef  OKAlertView_BtnBgFormateBorderColor
#define  OKAlertView_BtnBgFormateBorderColor             [OKColorFromHex(0x999999) colorWithAlphaComponent:1.0]
#endif
...
#define  OKAlertView_BigBtnHeight               36.0f
#endif


//弹框离屏幕边缘宽度
#ifndef  OKAlertView_ScreenSpace
#define  OKAlertView_ScreenSpace    ([UIScreen mainScreen].bounds.size.width<375 ? 25 : 36)
#endif
...
//水平按钮的间距
#define  OKAlertView_HorizontalSpace           12
//线条高度
#define  OKAlertView_LineHeight                 (1/[UIScreen mainScreen].scale)
//线条颜色
#define  OKAlertView_LineColor                  OKColorFromHex(0xDDDDDD)
//文本颜色
#define  OKAlertView_LabelColor                 [OSSVThemesColors col_0D0D0D]

//文本颜色
#define  OKAlertView_ContentColor               OKColorFromHex(0x666666)
...
/** AlertView普通按钮点击回调 */
@property (nonatomic, copy) STLAlertViewCallBackBlock       alertCallBackBlock;
/** AlertView取消按钮点击回调 */
@property (nonatomic, copy) STLAlertViewCancelBlock         cancelButtonBlock;
/** AlertView输入框弹窗回调*/
@property (nonatomic, copy) STLAlertViewInputCallBackBlock  inputViewCallBackBlock;
/** AlertView取消按钮标题 */
             ...
             otherButtonTitles:(id)otherButtonTitles, ...NS_REQUIRES_NIL_TERMINATION
{
    ...
    return [[STLAlertView alloc] initWithFrame:rect
                                        title:title
                                      message:message
...
{
    BOOL canShow = [STLAlertView judgeCanShowAlert:cancelButtonTitle
                                          message:message
                                            title:title];
    if(!canShow) return nil;
...
STLAlertView* ShowAlertViewFormatteButtonAlignment(STLAlertViewFormateType buttonFormate, id title, id message, NSArray *otherButtonTitles, NSTextAlignment messageAlignment, BOOL isAr, STLAlertViewCallBackBlock otherButtonBlock, id cancelButtonTitle, STLAlertViewCancelBlock cancelButtonBlock) {
    ...
    CGRect rect = [UIScreen mainScreen].bounds;
    return [[STLAlertView alloc] initWithFrameAlignment:rect
                       formate:buttonFormate
                    isVertical:NO
                         title:title
                       message:message
...
STLAlertView* ShowVerticalAlertView(id title, id message, NSArray *otherButtonTitles, STLAlertViewCallBackBlock otherButtonBlock, id cancelButtonTitle, STLAlertViewCancelBlock cancelButtonBlock)
{
    ...
    CGRect rect = [UIScreen mainScreen].bounds;
    return [[STLAlertView alloc] initWithFrame:rect
                                       formate:STLAlertViewFormateTypeNormal
                                   isVertical:YES
                                        title:title
                                      message:message
                                      isNeedAr:NO
...
void ShowAlertSingleBtnView(id title, id message, id cancelButtonTitle){
    [STLAlertView alertWithTitle:title
                        message:message
              cancelButtonTitle:cancelButtonTitle
...
- (instancetype)initWithFrame:(CGRect)frame
                      formate:(STLAlertViewFormateType)buttonFormateType
                   isVertical:(BOOL)isVertical
                        title:(id)title
                      message:(id)message
                     isNeedAr:(BOOL)isAr
            otherButtonTitles:(NSArray *)buttonTitleArr
             otherButtonBlock:(STLAlertViewCallBackBlock)alertWithCallBlock
            ...
            cancelButtonBlock:(STLAlertViewCancelBlock)cancelButtonBlock
{
    ...
    if(self){
        ...
        //设置控件主题色
        self.titleTextAttributes = [STLAlertView appearance].titleTextAttributes ? : nil;
        self.messageTextAttributes = [STLAlertView appearance].messageTextAttributes ? : nil;
        self.otherBtnTitleAttributes = [STLAlertView appearance].otherBtnTitleAttributes ? : nil;
        self.themeColorBtnTitleAttributes = [STLAlertView appearance].themeColorBtnTitleAttributes ? : nil;
        
        //1.先移除window上已存在的OKAlertView
        [self removeOkAlertFromWindow];
        
        //2.初始化弹框标题和描述
        CGFloat lastLabMaxY = [self layoutTitleAndMessageUI:title message:message isNeedAr:isAr];
        ...
        if (allTitleArr.count > 0) {
            //3.布局所有弹框按钮
            [self layoutAlert:buttonFormateType
                 MutableBtnUI:allTitleArr
...
- (instancetype)initWithFrameAlignment:(CGRect)frame
                      formate:(STLAlertViewFormateType)buttonFormateType
                   isVertical:(BOOL)isVertical
                        title:(id)title
                      message:(id)message
            ...
            cancelButtonBlock:(STLAlertViewCancelBlock)cancelButtonBlock
{
    ...
    if(self){
        ...
        //2.初始化弹框标题和描述
        CGFloat lastLabMaxY = [self layoutTitleAndMessageAlignmentUI:title message:message messageAlignment:messageAlignment isNeedAr:isAr];
 ...
 * 初始化弹框主视图
 */
- (CGFloat)layoutTitleAndMessageUI:(id)title message:(id)message isNeedAr:(BOOL)isAr
{
    //弹框矩形视图
    ...
    if (hasContent) {
        ...
        CGFloat titleHeight = [STLAlertView calculateTextHeight:titleLab.font
    ...
    if (message) {
        ...
        UIFont *messageFont = hasContent ? OKFONTDEFAULT(13) : OKFONTDEFAULT(14);
        UIColor *messageColor = hasContent ? OKAlertView_ContentColor : OKAlertView_LabelColor;
        messageLab = [[UILabel alloc] init];
        messageLab.backgroundColor = [UIColor clearColor];
        [messageLab setTextColor:messageColor];
        [messageLab setFont:messageFont];
        ...
        if ([message isKindOfClass:[NSString class]]) {
            ...
            if (self.messageTextAttributes) {
                ...
                // 字体: 大小 颜色 行间距
                NSAttributedString *messageAttr = [[NSAttributedString alloc]initWithString:message attributes:@{NSFontAttributeName:messageFont,NSForegroundColorAttributeName:messageColor,NSParagraphStyleAttributeName:paragraph}];
                msgObject = messageAttr;
                [messageLab setAttributedText:messageAttr];
        ...
        CGFloat messageMaxHeight = [UIScreen mainScreen].bounds.size.height * 0.44;

        CGFloat msgHeight = [STLAlertView calculateTextHeight:messageLab.font
                                          constrainedToWidth:labelWidth
                                                  textObject:msgObject];
        CGFloat messageLabY = hasContent ? (lastLabMaxY+OKAlertView_KitMargin*0.5) : spaceY;
        ...
        if (isAr && [OSSVSystemsConfigsUtils isRightToLeftShow]) {
 ...
 * 初始化弹框主视图
 */
- (CGFloat)layoutTitleAndMessageAlignmentUI:(id)title message:(id)message messageAlignment:(NSTextAlignment)messageAlignment isNeedAr:(BOOL)isAr
{
    //弹框矩形视图
    CGFloat contentW = OKFullScreenWidth-OKAlertView_ScreenSpace*2;
    CGRect rect = CGRectMake(OKAlertView_ScreenSpace, 0, contentW, 0);
    ...
    if (message) {
        ...
        messageLab.textAlignment = messageAlignment;
 ...
 * 布局所有Alert按钮
 */
- (void)layoutAlert:(STLAlertViewFormateType)buttonFormateType
       MutableBtnUI:(NSArray *)allTitleArr
              ...
              buttonAction:(SEL)selector
{
    ...
    if (buttonFormateType == STLAlertViewFormateTypeButton || buttonFormateType == STLAlertViewFormateTypeButtonColumn) {
        
        CGFloat buttonAllContentW = contentW - OKAlertView_KitMargin*2;
        ...
        for (int i = 0 ; i<allTitleArr.count; i++) {
            ...
            [actionBtn.titleLabel setFont:OKFONTDEFAULT_BUTTON(12)];
            [actionBtn setTitleColor:OKAlertView_BtnTitleFormateNorColor forState:0];
...
//            [actionBtn setBackgroundImage:[STLAlertView ok_imageWithColor:OKAlertView_BtnBgDisabledColor] forState:UIControlStateDisabled];
//            [actionBtn setBackgroundImage:[STLAlertView ok_imageWithColor:OKAlertView_BtnBgHighColor] forState:UIControlStateHighlighted];
            [actionBtn setExclusiveTouch:YES];
            
            if (i==allTitleArr.count-1) {
                [contentView addSubview:actionBtn];
                [actionBtn setTitleColor:OKAlertView_BtnTitleFormateHighColor forState:0];
                [actionBtn setBackgroundImage:[STLAlertView ok_imageWithColor:OKAlertView_BtnBgFormateHighColor] forState:UIControlStateNormal];
            } else {
                actionBtn.layer.borderWidth = 1.0;
                actionBtn.layer.borderColor = OKAlertView_BtnBgFormateBorderColor.CGColor;
            ...
            CGFloat lineY = lastLabMaxY;

            //按钮个数大于2个  上下排列，第一个背景黑色
            if (allTitleArr.count > 2 || buttonFormateType == STLAlertViewFormateTypeButtonColumn) {
                ...
                if (i == 0) {
                    ...
                    [actionBtn setTitleColor:OKAlertView_BtnTitleFormateNorColor forState:0];
                    [actionBtn setBackgroundImage:[STLAlertView ok_imageWithColor:[OSSVThemesColors stlWhiteColor]] forState:UIControlStateNormal];
                    actionBtn.layer.borderColor = OKAlertView_BtnBgFormateBorderColor.CGColor;
                    actionBtn.layer.borderWidth = 1.0;


                }
                CGFloat btnY = lastBtnMaxY > 0 ? lastBtnMaxY : lineY;
                if (scrollView && i==0) {
                    btnY = 0;
                }
                actionBtn.frame = CGRectMake(OKAlertView_KitMargin, btnY, buttonAllContentW, OKAlertView_BigBtnHeight);

                lastBtnMaxY = CGRectGetMaxY(actionBtn.frame) + OKAlertView_KitMargin / 2.0;
                
            } else {
                CGFloat btnY = lastLabMaxY;
                CGFloat btnW = allTitleArr.count==2 ? (buttonAllContentW - OKAlertView_HorizontalSpace)/2 : buttonAllContentW;
               
                actionBtn.frame = CGRectMake(OKAlertView_KitMargin + btnW * i + OKAlertView_HorizontalSpace * i, btnY, btnW, OKAlertView_BigBtnHeight);

            }
            
            if (scrollView) {
                if (i == allTitleArr.count-1) {
                    scrollView.contentSize = CGSizeMake(contentW, CGRectGetMaxY(actionBtn.frame) + OKAlertView_KitMargin);
                }
            } else {
                contentView.bounds = CGRectMake(0, 0, contentW, CGRectGetMaxY(actionBtn.frame) + OKAlertView_KitMargin);
            }
            contentView.center = self.center;
        }
        ...
        for (int i = 0 ; i<allTitleArr.count; i++) {
            ...
            UIButton *actionBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            actionBtn.backgroundColor = [UIColor whiteColor];
            [actionBtn.titleLabel setFont:OKFONTDEFAULT_BUTTON(16)];
    ...
    NSString *saveKey = @"hasScrolledSTLAlertView";
...
- (void)inputButtonAction:(UIButton *)sender
{
    ...
    BOOL isvalidEmail = [OSSVNSStringTool isValidEmailString:textField.text];
    if (isvalidEmail) {
...
void ShowAlertToastDelay(id title, id message, NSTimeInterval duration, void(^dismissBlock)(void)){
    if (!title && !message) return;
    STLAlertView *alertView = [STLAlertView alertWithTitle:title
                                                 message:message
                                       cancelButtonTitle:nil
                                       cancelButtonBlock:nil
                                        otherButtonBlock:nil
...
- (void)removeOkAlertFromWindow
{
    ...
    for (UIView *windowSubView in window.subviews) {
        if ([windowSubView isKindOfClass:[STLAlertView class]]) {
                               ...
                               cancelBlock:(void (^)(void))cancelBlock
{
    ...
    NSDictionary *cancelAttributes = [STLAlertView appearance].themeColorBtnTitleAttributes;
    ...
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] < 9.0) {
            ...
            if([textField isKindOfClass:cls] && [STLAlertView verifyObject:cls ok_hasVarName:@"_textFieldView"]) {
    ...
    /** 是否能获取该属性*/
    if(title && [STLAlertView verifyObject:alertController ok_hasVarName:@"attributedTitle"])
        {
        //设置标题为细体
    ...
    //设置按钮颜色
    if([STLAlertView verifyObject:[UIAlertAction class] ok_hasVarName:@"titleTextColor"])
        {
...
{
    STLAlertView *alerView = [[STLAlertView alloc] initBindEmailAlertViewTitle:title
                                                                     message:message
                                 ...
                                 completion:(STLAlertViewInputCallBackBlock)completion
{
    ...
    if (self) {
        ...
        //3.添加inputView
        CGFloat padding = 14;
        STLTextField *textField = [[STLTextField alloc] initWithFrame:CGRectMake(padding, lastLabMaxY, contentW - padding * 2, 36)];
        textField.leftPadding = 10;
        textField.placeholder = placeHolder;
        textField.delegate = self;
        textField.tag = OKAlertView_InputTag;
        textField.layer.borderWidth = 1;
        textField.layer.borderColor = [OSSVThemesColors col_CCCCCC].CGColor;
        ...
        errorLabel.textColor = OSSVThemesColors.col_FE5269;
        ...
        [self layoutAlert:STLAlertViewFormateTypeNormal MutableBtnUI:allTitleArr contentView:self.contentView lastUImaxY:lastLabMaxY isVertical:NO buttonAction:@selector(inputButtonAction:)];
