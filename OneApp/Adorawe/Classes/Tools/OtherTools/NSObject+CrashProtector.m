...
{
    if (!CP_OPEN) {
        DLog(@"CrashProtector  close !");
        return;
    }
    DLog(@"CrashProtector  open !");
    [self openCP];
}
...
+ (BOOL)swizzlingClass:(Class)klass replaceClassMethod:(SEL)methodSelector1 withMethod:(SEL)methodSelector2
{
    if (!klass || !methodSelector1 || !methodSelector2) {
        DLog(@"Nil Parameter(s) found when swizzling.");
        return NO;
    }
    ...
    if (method1 && method2) {
        ...
        DLog(@"Swizzling Method(s) not found while swizzling class %@.", NSStringFromClass(klass));
        return NO;
    }
}
...
- (instancetype)qiye_initWithObjects:(const id  _Nonnull __unsafe_unretained *)objects forKeys:(const id<NSCopying>  _Nonnull __unsafe_unretained *)keys count:(NSUInteger)cnt{
    if (cnt == 0) {
        return [self qiye_initWithObjects:nil forKeys:nil count:0];
    }
    id safeObjects[cnt];
    id safeKeys[cnt];
...
{
    if (cnt == 0) {
        return [self qiye_dictionaryWithObjects:nil forKeys:nil count:0];
    }
    id safeObjects[cnt];
    id safeKeys[cnt];
...
{
    if (cnt == 0) {
        return [self qiye_initWithObjects:nil count:0];
    }
    id safeObjects[cnt];
    NSUInteger j = 0;
    for (NSUInteger i = 0; i < cnt ; i++) {
...
- (void)observeValueForKeyPath:(nullable NSString *)keyPath ofObject:(nullable id)object change:(nullable NSDictionary<NSKeyValueChangeKey, id> *)change context:(nullable void *)context{
    DLog(@"KVOProxy - observeValueForKeyPath :%@",change);
    __block CPKVOInfo *info ;
    {
        [self lock];
