...
+ (void)load
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        [obj swizzleInstanceMethod:@selector(subarrayWithRange:) withMethod:@selector(hookSubarrayWithRange:)];
        [obj swizzleInstanceMethod:@selector(removeObjectsAtIndexes:) withMethod:@selector(hookRemoveObjectsAtIndexes:)];
        
        //setObject:atIndexedSubscript:
        [obj swizzleInstanceMethod:@selector(setObject:atIndexedSubscript:) withMethod:@selector(hookObject:atIndexedSubscript:)];

        [obj swizzleInstanceMethod:@selector(getObjects:range:) withMethod:@selector(hookObjects:range:)];
...
- (void)hookRemoveObjectsAtIndexes:(NSIndexSet *)indexSet {    
    @try {
        [self hookRemoveObjectsAtIndexes:indexSet];
    }
    @catch (NSException *exception) {
        SFAssert(NO, @"NSMutableArray invalid args hookRemoveObjectsAtIndexes:[%i--%i]", indexSet.firstIndex,indexSet.lastIndex);
    }
    @finally {
    }
}

- (void)hookObject:(id)obj atIndexedSubscript:(NSUInteger)idx {
    @try {
        [self hookObject:obj atIndexedSubscript:idx];
    }
    @catch (NSException *exception) {
        SFAssert(NO, @"NSMutableArray setindex [%@] over all count:%@", @(idx),@(self.count));
    }
    @finally {
    }
}

- (void)hookObjects:(__unsafe_unretained id  _Nonnull *)objects range:(NSRange)range {
    @try {
        [self hookObjects:objects range:range];
    } @catch (NSException *exception) {
    } @finally {
    }
}
