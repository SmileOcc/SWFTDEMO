...
@implementation STLAnimateTextField
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.textAlignment = [OSSVSystemsConfigsUtils isRightToLeftShow] ? NSTextAlignmentRight : NSTextAlignmentLeft;;
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeEditing) name:kNotif_ShowPlaceholderAnimation object:nil];
    }
    return self;
}
...
- (UILabel *)placeholderAnimationLabel {
    if (!_placeholderAnimationLabel) {
        _placeholderAnimationLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH*0.7, 30)];
        _placeholderAnimationLabel.backgroundColor = [UIColor clearColor];
        _placeholderAnimationLabel.userInteractionEnabled = NO;
        _placeholderAnimationLabel.textColor = [OSSVThemesColors col_0D0D0D];
    }
