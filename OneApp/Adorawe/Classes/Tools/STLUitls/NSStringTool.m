...
+ (id)desEncrypt:(STLBaseRequest *)request {
    if (!request) {
        DLog(@"--------------XXXXXX 请求不存在❎");
        return nil;
    }
...
+(NSString*)desEncryptToString:(STLBaseRequest *)request{
    if (!request) {
        DLog(@"--------------XXXXXX 请求不存在❎");
        return nil;
    }
...
+ (NSString*)jsonStringWithDict:(NSDictionary *)dict {
    NSError *error;
    NSData *jsonData = [STLJSONSerialization dataWithJSONObject:dict
