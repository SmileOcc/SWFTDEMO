...
+ (void)advEventTarget:(id)target withEventModel:(OSSVAdvsEventsModel *)advEventModel {
    ...
    [OSSVAccountsManager testLaunchMessage:[NSString stringWithFormat:@"advEventTarget:%@ url:%@,hasNav:%@",NSStringFromClass(targetVC.class),url,targetVC.navigationController ? @"1" : @"0"]];
    ...
    switch (actionType) {
        ...
        case AdvEventTypeInsertH5: // embedPage 嵌入H5
        {
            ...
            NSString *userToken = OSSVAccountsManager.sharedManager.isSignIn ? STLToString(USER_TOKEN) : @"";
...
+ (void)jumpDeeplinkTarget:(id)target deeplinkParam:(NSDictionary *)paramDict {
    OSSVAdvsEventsModel *eventModel = [[OSSVAdvsEventsModel alloc] init];
...
+ (void)saveNotificationsPaymentParmaters:(NSDictionary *)userInfo {
    if ([userInfo isKindOfClass:[NSDictionary class]]) {
        ...
        [us setInteger:[[OSSVNSStringTool getCurrentTimestamp] integerValue] forKey:SAVE_NOTIFICATIONS_PARMATERS_TIME]; // 推送时间
...
+ (void)saveOneLinkeParams:(NSDictionary *)paramDict {
    ...
    if ([paramDict isKindOfClass:[NSDictionary class]]) {        
        if (!STLIsEmptyString(paramDict[@"utm_source"]) ||
            ...
            NSString *timeStr = [OSSVNSStringTool getCurrentTimestamp];
            NSUserDefaults *us = [NSUserDefaults standardUserDefaults];
