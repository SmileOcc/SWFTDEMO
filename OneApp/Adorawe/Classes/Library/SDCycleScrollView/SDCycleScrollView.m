...
- (void)initialization
{
    ...
    _pageControlBottomNewOffset = 0;
    _pageControlRightOffset = 0;
    _pageControlRightNewOffset = 0;
...
{
#pragma mark  这里打开会影响到背景图和图片的展位图重叠
...
- (void)setCustomPageControlDotImage:(UIImage *)image isCurrentPageDot:(BOOL)isCurrentPageDot
{
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        TAPageControl *pageControl = (TAPageControl *)_pageControl;
        if (APP_TYPE == 3) {
            pageControl.spacingBetweenDots = 3;
        }
        if (isCurrentPageDot) {
...
#pragma mark ////10010x修改
- (void)setTitlesAttrGroup:(NSArray *)titlesAttrGroup {
    _titlesAttrGroup = titlesAttrGroup;
    if (self.onlyDisplayText) {
        NSMutableArray *temp = [NSMutableArray new];
        for (int i = 0; i < _titlesAttrGroup.count; i++) {
            [temp addObject:@""];
        }
        self.backgroundColor = [UIColor clearColor];
        self.imageURLStringsGroup = [temp copy];
...
#pragma mark //适配iOS14
        //occcc UIpagecontrol圆点不显示解决方案
        if (iosVersion >= 14.0 && [self.pageControl isKindOfClass:[UIPageControl class]]) {
            UIPageControl *page = (UIPageControl *)self.pageControl;
            size.width = [page sizeForNumberOfPages:self.imagePathsGroup.count].width;
        }
    }
...
#pragma mark    ////10010修改
    ...
    if (_scrollDirection == UICollectionViewScrollDirectionVertical) {
        ...
        y = [OSSVSystemsConfigsUtils isRightToLeftShow] ? _pageControlBottomOffset * 2 + size.height : self.mainView.sd_height - size.height;//10;
    ...
    pageControlFrame.origin.y-=self.pageControlBottomOffset - self.pageControlBottomNewOffset;
    pageControlFrame.origin.x-=self.pageControlRightOffset - self.pageControlRightNewOffset;
...
#pragma mark        ////10010修改
    ...
    if (_titlesAttrGroup.count && itemIndex < _titlesAttrGroup.count) {
        [cell setTitleAttribute:_titlesAttrGroup[itemIndex]];
    }
    
#pragma mark ////10010修改
    if (_scrollDirection == UICollectionViewScrollDirectionVertical) {
        if ([OSSVSystemsConfigsUtils isRightToLeftShow]) {
            cell.titleLabelTextAlignment = NSTextAlignmentRight;
            //[cell setTitleLineBreakMode:NSLineBreakByTruncatingHead];
        }
        
        [cell setTitleLabelumberOfLines:2];
    ...
    if (self.titleLabelNmberOfLines > 0) {
        [cell setTitleLabelumberOfLines:self.titleLabelNmberOfLines];
    }
