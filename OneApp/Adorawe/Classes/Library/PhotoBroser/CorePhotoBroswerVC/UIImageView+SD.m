...
-(void)imageWithUrlStr:(NSString *)urlStr phImage:(UIImage *)phImage{
    ...
    [self yy_setImageWithURL:url placeholder:phImage];
...
-(void)imageWithUrlStr:(NSString *)urlStr phImage:(UIImage *)phImage progressBlock:(YYWebImageProgressBlock)progressBlock completedBlock:(YYWebImageCompletionBlock)completedBlock {
    if(urlStr==nil) return;
    
    NSURL *url=[NSURL URLWithString:urlStr];
    YYWebImageOptions options = YYWebImageOptionShowNetworkActivity;
    [self yy_setImageWithURL:url placeholder:phImage options:options progress:progressBlock transform:nil completion:completedBlock];
}
