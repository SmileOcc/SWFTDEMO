...
- (void)viewDidLoad {
    ...
    CGFloat topSpace = [[UIScreen mainScreen]bounds].size.height > 736.0 ? 24 : 0;
    _topBarHeightC.constant = topSpace;
}
...
-(void)singleTap{
    ...
    CGFloat topSpace = [[UIScreen mainScreen]bounds].size.height > 736.0 ? 24 : 0;
    _topBarHeightC.constant = show?-h:topSpace;
...
- (IBAction)rightBtnClick:(id)sender {
    ...
    if(!itemView.hasImage){
        [CoreSVP showSVPWithType:CoreSVPTypeError Msg:@"No Image" duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
        return;
    }
    ...
    if([itemModel read]){//已经保存过本地
        ...
        [CoreSVP showSVPWithType:CoreSVPTypeInfo Msg:@"Image Saved" duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
    }else{
        
        //展示提示框
        [CoreSVP showSVPWithType:CoreSVPTypeLoadingInterface Msg:@"Saving..." duration:0 allowEdit:NO beginBlock:nil completeBlock:nil];
        
        [itemView save:^{
            
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.5f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                
                [CoreSVP showSVPWithType:CoreSVPTypeSuccess Msg:@"Save Successful" duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
            ...
            [CoreSVP showSVPWithType:CoreSVPTypeError Msg:@"Save Fail" duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
        }];
    }
}
...
//        STLLog(@"currentItemView:%@",self.currentItemView);
