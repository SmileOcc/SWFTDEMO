...
- (void)setup{
    ...
    slideView_.enableScroller = self.enableScroller;
    [self addSubview:slideView_];
}

-(void)setEnableScroller:(BOOL)enableScroller
{
    _enableScroller = enableScroller;
    
    slideView_.enableScroller = _enableScroller;
}
...
//    if (percent == 1.0) {
//        // 等于滑动结束的时候,到的新界面，可能添加统计
//        STLLog(@"滑动的时候 precent == %f，new Index == %lu",percent,toIndex);
//    }
}
