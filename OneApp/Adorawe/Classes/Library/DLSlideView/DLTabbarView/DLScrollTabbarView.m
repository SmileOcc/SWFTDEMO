...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    trackView_.frame = CGRectMake(x - (_trackLineViewMoreWidth/2.0), trackView_.frame.origin.y, width +_trackLineViewMoreWidth, CGRectGetHeight(trackView_.bounds));
}
...
//            }
            ...
            // 此处增加了 线的长度
            trackView_.frame = CGRectMake(trackRc.origin.x - (_trackLineViewMoreWidth / 2.0), trackView_.frame.origin.y, trackRc.size.width + _trackLineViewMoreWidth, CGRectGetHeight(trackView_.bounds));
        }
