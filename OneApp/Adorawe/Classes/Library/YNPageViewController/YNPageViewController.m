...
- (instancetype)initPageViewControllerWithControllers:(NSArray *)controllers
    ...
    self.controllersM = controllers.mutableCopy;
    self.titlesM = titles.mutableCopy;
    self.subTitlesM = subTitles.mutableCopy;
    self.config = config ?: [YNPageConfigration defaultConfig];
    self.displayDictM = @{}.mutableCopy;
    self.cacheDictM = @{}.mutableCopy;
    self.originInsetBottomDictM = @{}.mutableCopy;
...
- (void)initPagescrollMenuViewWithFrame:(CGRect)frame {
    ...
    if (self.subTitlesM.count > 0) {
        scrollMenuView = [YNPageScrollMenuView pagescrollMenuViewWithFrame:frame
                                                                    titles:self.titlesM
                                                                  subTitles:self.subTitlesM
...
- (void)initViewControllerWithIndex:(NSInteger)index {
    ...
    title = [NSString stringWithFormat:@"%@%ld",[self titleWithIndex:index],(long)index];

    //////occcccc 修改
    if (self.subTitlesM.count > index) {
        title = [NSString stringWithFormat:@"%@_%@",title,STLToString(self.subTitlesM[index])];
    }
...
- (void)addViewControllerToParent:(UIViewController *)viewController index:(NSInteger)index {
    ...
    NSString *titleKey = [self getKeyWithTitle:title];
    if (titleKey.length>0) {
        [self.displayDictM setObject:viewController forKey:titleKey];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.topViewHasDidScrollView) { //回调顶部拖动回调
        self.topViewHasDidScrollView(scrollView.contentOffset.y);
    }
...
- (void)yn_pageScrollViewDidScrollView:(UIScrollView *)scrollView {
    ...
    if ([self isSuspensionBottomStyle] || [self isSuspensionTopStyle]) {
        ...
        title = [NSString stringWithFormat:@"%@%ld",title,(long)index];
...
- (void)pagescrollMenuViewItemOnClick:(UIButton *)label index:(NSInteger)index {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(pagescrollMenuViewItemOnClick:index:)]) {
        [self.delegate pagescrollMenuViewItemOnClick:label index:index];
    }
}
...
- (void)updateMenuItemTitle:(NSString *)title index:(NSInteger)index {
    ...
    oldTitle = [NSString stringWithFormat:@"%@%ld",oldTitle,(long)index];
...
- (void)setupPageScrollView {
    ...
    if ([self isSuspensionTopPauseStyle]) {
        ...
        self.bgScrollView.contentSize = CGSizeMake(kYNPAGE_SCREEN_WIDTH, self.config.hideContentView ? self.headerBgView.yn_height - self.config.suspenOffsetY : contentHeight + self.headerBgView.yn_height - self.config.suspenOffsetY);
...
- (void)removeViewController {
    NSString *title = [self titleWithIndex:self.pageIndex];
    title = [NSString stringWithFormat:@"%@%ld",[self titleWithIndex:self.pageIndex],(long)self.pageIndex];

    //////occcccc 修改
    if (self.subTitlesM.count > self.pageIndex) {
        title = [NSString stringWithFormat:@"%@_%@",title,STLToString(self.subTitlesM[self.pageIndex])];
    }
