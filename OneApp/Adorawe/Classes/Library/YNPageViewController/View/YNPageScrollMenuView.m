...
/// items
@property (nonatomic, strong) NSMutableArray<UIButton *> *itemsSubArrayM;
/// item宽度
...
+ (instancetype)pagescrollMenuViewWithFrame:(CGRect)frame
    ...
    menuView.itemsSubArrayM = @[].mutableCopy;
...
- (void)setupItems {
    ...
    if (self.configration.buttonArray.count > 0 && self.titles.count == self.configration.buttonArray.count) {
        [self.configration.buttonArray enumerateObjectsUsingBlock:^(UIButton * _Nonnull itemButton, NSUInteger idx, BOOL * _Nonnull stop) {
            [self setupButton:(YNPageScrollMenuButton *)itemButton title:self.titles[idx] idx:idx];
        }];
    } else {
        [self.titles enumerateObjectsUsingBlock:^(id  _Nonnull title, NSUInteger idx, BOOL * _Nonnull stop) {
            YNPageScrollMenuButton *itemButton = [YNPageScrollMenuButton buttonWithType:UIButtonTypeCustom];
            [self setupButton:itemButton title:title idx:idx];
        }];
    }
}

- (void)setupButton:(YNPageScrollMenuButton *)itemButton title:(NSString *)title idx:(NSInteger)idx {
    ...
    [itemButton setBackgroundColor:self.configration.normalItemBackgroundColor forState:UIControlStateNormal];
    [itemButton setTitle:title forState:UIControlStateNormal];
    if (self.subTitles.count > idx) {
        itemButton.btnType = YNPageScrollMenuButtonTypeSubTitle;
        itemButton.subTitle.text = self.subTitles[idx];
        itemButton.subTitle.textColor = self.configration.normalItemColor;
    }
    itemButton.tag = idx;
...
- (void)setDefaultTheme {
    ...
    YNPageScrollMenuButton *occMenuButton = nil;
    if ([currentButton isKindOfClass:[YNPageScrollMenuButton class]]) {
        occMenuButton = (YNPageScrollMenuButton *)currentButton;
    }
    /// 缩放
    if (self.configration.itemMaxScale > 1) {
    ...
    [currentButton setBackgroundColor:self.configration.selectedItemBackgroundColor forState:UIControlStateNormal];
    currentButton.titleLabel.font = self.configration.selectedItemFont;
    
    //////occ修复
    if (occMenuButton) {
        occMenuButton.subTitle.textColor = self.configration.selectedItemColor;
    }
    /// 线条
    if (self.configration.showScrollLine) {
...
- (void)adjustItemAnimate:(BOOL)animated {
    ...
    //////fffff测cc修改
    YNPageScrollMenuButton *occMenuButtonLast = nil;
    if ([lastButton isKindOfClass:[YNPageScrollMenuButton class]]) {
        occMenuButtonLast = (YNPageScrollMenuButton *)lastButton;
    }
    
    YNPageScrollMenuButton *occMenuButtonCurrnet = nil;
    if ([currentButton isKindOfClass:[YNPageScrollMenuButton class]]) {
        occMenuButtonCurrnet = (YNPageScrollMenuButton *)currentButton;
    }
    ...
    [UIView animateWithDuration:animated ? 0.3 : 0 animations:^{
        ...
        [self.itemsArrayM enumerateObjectsUsingBlock:^(UIButton  * obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [obj setBackgroundColor:self.configration.normalItemBackgroundColor forState:UIControlStateNormal];
            obj.titleLabel.font = self.configration.itemFont;
            
            if ([obj isKindOfClass:[YNPageScrollMenuButton class]]) {
                YNPageScrollMenuButton *occTestObj = (YNPageScrollMenuButton *)obj;
                occTestObj.subTitle.textColor = self.configration.normalItemColor;
            }
            if (idx == self.itemsArrayM.count - 1) {
                ...
                if (occMenuButtonCurrnet) {
                    occMenuButtonCurrnet.subTitle.textColor = self.configration.selectedItemColor;
                }
            }
        }];
...
- (void)adjustItemWithProgress:(CGFloat)progress
    ...
    if (self.configration.showGradientColor) {
        ...
        if (occMenuButtonLast) {
            occMenuButtonLast.subTitle.textColor = norColor;
        }
        if (occMenuButtonCurrnet) {
            occMenuButtonCurrnet.subTitle.textColor = selColor;
        }
    } else{
        if (progress > 0.5) {
            [lastButton setTitleColor:self.configration.normalItemColor forState:UIControlStateNormal];
            [lastButton setBackgroundColor:self.configration.normalItemBackgroundColor forState:UIControlStateNormal];
            [currentButton setTitleColor:self.configration.selectedItemColor forState:UIControlStateNormal];
            [currentButton setBackgroundColor:self.configration.selectedItemBackgroundColor forState:UIControlStateNormal];
            currentButton.titleLabel.font = self.configration.selectedItemFont;
            
            if (occMenuButtonLast) {
                occMenuButtonLast.subTitle.textColor = self.configration.normalItemColor;
            }
            if (occMenuButtonCurrnet) {
                occMenuButtonCurrnet.subTitle.textColor = self.configration.selectedItemColor;
            }
            ...
            [lastButton setBackgroundColor:self.configration.selectedItemBackgroundColor forState:UIControlStateNormal];
            ...
            [currentButton setBackgroundColor:self.configration.normalItemBackgroundColor forState:UIControlStateNormal];
            currentButton.titleLabel.font = self.configration.itemFont;
            
            if (occMenuButtonLast) {
                occMenuButtonLast.subTitle.textColor = self.configration.selectedItemColor;
            }
            if (occMenuButtonCurrnet) {
                occMenuButtonCurrnet.subTitle.textColor = self.configration.normalItemColor;
            }
