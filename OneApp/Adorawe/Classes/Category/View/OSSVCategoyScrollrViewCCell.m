...
- (void)cycleScrollView:(SDCycleScrollView *)cycleScrollView didSelectItemAtIndex:(NSInteger)index
{
    if (self.model && self.model.banner.count > index) {
        ...
        OSSVAdvsEventsModel *advEventModel = [self.model.banner objectAtIndex:index];
...
{
    CGFloat imageH = [OSSVCategoryssViewsModel secondRangeCollectionWidth] * 100.0 / 254.0;
...
- (void)guidClick:(UIButton *)btns {
    if (self.model && self.model.guide.count > ((int)btns.tag - 10)) {
        ...
        OSSVAdvsEventsModel *guideModel = [self.model.guide objectAtIndex:(int)btns.tag - 10];
        if(self.categoriesClickDelegate && [self.categoriesClickDelegate respondsToSelector:@selector(categoriesCell:advModel:isBanner:)]) {
...
+ (CGFloat)scrollViewContentH:(OSSVCategorysModel *)model {
    ...
    if (model.banner.count != 0) {
        // 10 顶部间隙（广告）
        topBannerH = 12;
        bannerHeight = [OSSVCategoryssViewsModel secondRangeCollectionWidth] * 100.0 / 254.0;
    }
    if (model.guide.count != 0) {
        
        CGSize itemSize = [OSSVCategoryssViewsModel secondRangeItemSize];
        // 10 与上面广告间隙
        if (APP_TYPE == 3) {
...
-(void)setModel:(OSSVCategorysModel *)model {
    ...
    if (model) {        
        ...
        if (model.banner.count == 0) {
            ...
            for (OSSVAdvsEventsModel *bannerModel in model.banner) {
        ...
        if (model.guide.count != 0) {
            ...
            for (int i = 0; i < model.guide.count; i ++) {
                OSSVAdvsEventsModel *guideModel = [model.guide objectAtIndex:i];
...
- (UIView *)collectBackGroundView {
    if (!_collectBackGroundView) {
        ...
        _collectBackGroundView.backgroundColor = [OSSVThemesColors col_F5F5F5];
        _collectBackGroundView.userInteractionEnabled = YES;
        if (APP_TYPE == 3) {
            _collectBackGroundView.backgroundColor = [OSSVThemesColors stlWhiteColor];
        }
    }
...
- (UIView *)cycleContentView {
    if (!_cycleContentView) {
        ...
        _cycleContentView.backgroundColor = [OSSVThemesColors stlWhiteColor];
        if (APP_TYPE == 3) {
...
//        _cycleScrollView = [SDCycleScrollView cycleScrollViewWithFrame:CGRectZero delegate:self placeholderImage:[UIImage imageNamed:@"placeholder_banner_pdf"]];
        ...
        _cycleScrollView.backgroundColor = [OSSVThemesColors col_EEEEEE];
        _cycleScrollView.autoScrollTimeInterval = 3.0; // 间隔时间
//        _cycleScrollView.currentPageDotColor = [OSSVThemesColors col_262626];
...
//        _cycleScrollView.pageDotColor = OSSVThemesColors.col_F1F1F1;
...
- (UIView *)guidContentView {
    if (!_guidContentView) {
        ...
        _guidContentView.backgroundColor = [OSSVThemesColors stlWhiteColor];
        if (APP_TYPE == 3) {
...
- (UILabel *)featuredLabel
{
    if (!_featuredLabel)
    {
        ...
        self.featuredLabel.backgroundColor = [OSSVThemesColors stlWhiteColor];
        self.featuredLabel.textColor = [OSSVThemesColors stlBlackColor];
        if (APP_TYPE == 3) {
...
- (instancetype)initWithFrame:(CGRect)frame;
{
    if (self = [super initWithFrame:frame]) {
        ...
        featuredImageView.backgroundColor = [OSSVThemesColors stlWhiteColor];
        ...
        featuredtitleLabel.textColor = [OSSVThemesColors col_6C6C6C];
