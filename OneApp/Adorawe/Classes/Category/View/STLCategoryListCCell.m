...
@property (nonatomic, strong) STLGoodsPriceView     *priceView; // 底部背景View
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        self.contentView.backgroundColor = [STLThemeColor stlWhiteColor];
        ...
        _priceView = [[STLGoodsPriceView alloc] initWithFrame:CGRectZero isShowIcon:YES];
...
//                                                    image = [image yy_imageByResizeToSize:CGSizeMake(kGoodsWidth, kGoodsWidth / model.goodsImageWidth * model.goodsImageHeight) contentMode:UIViewContentModeScaleAspectFit];
    ...
    self.priceView.priceLabel.textColor = [STLThemeColor col_0D0D0D];
    ...
    if (isDiscount)
    {
        ...
        if (APP_TYPE == 3) {
            self.priceView.priceLabel.textColor = STLThemeColor.col_9F5123;
            self.priceView.originalPriceLabel.hidden = NO;

        } else {
            self.priceView.priceLabel.textColor = STLThemeColor.col_B62B21;
        }
    }
...
+ (CGFloat)categoryListCCellRowHeightForListModel:(OSSVCategoriyDetailsGoodListsModel *)model {
    ...
    if (model.goods_img_w > 0) {
        ...
        if (!(model.flash_sale && [model.flash_sale isOnlyFlashActivity]) && !model.hadHandlePriceHeight) {
            ...
            CGFloat activityHeight = [STLGoodsPriceView activithHeight:model.tips_reduction contentWidth:kGoodsWidth];
            fullHeight = [STLGoodsPriceView contentHeight:activityHeight];
...
+ (CGFloat)categoryListCCellRowHeightOneModel:(OSSVCategoriyDetailsGoodListsModel *)model twoModel:(OSSVCategoriyDetailsGoodListsModel *)twoModel{
    ...
    if (APP_TYPE == 3) {
        if (model.goods_img_w > 0) {
            ...
            if (!(model.flash_sale && [model.flash_sale isOnlyFlashActivity]) && !model.hadHandlePriceHeight) {
                model.hadHandlePriceHeight = YES;
                activityHeight = [STLGoodsPriceView activithHeight:model.tips_reduction contentWidth:kGoodsWidth];
                onefullHeight = [STLGoodsPriceView contentHeight:activityHeight];
...
//            }
            ...
            if (twoModel) {
                ...
                if (!(twoModel.flash_sale && [twoModel.flash_sale isOnlyFlashActivity]) && !twoModel.hadHandlePriceHeight) {
                    ...
                    twoActivityHeight = [STLGoodsPriceView activithHeight:twoModel.tips_reduction contentWidth:kGoodsWidth];
                    twofullHeight = [STLGoodsPriceView contentHeight:twoActivityHeight];
...
-(UILabel *)isNewLbl{
    if (!_isNewLbl) {
        ...
        if (APP_TYPE == 3) {
            ...
            _isNewLbl.backgroundColor = STLThemeColor.stlWhiteColor;
            _isNewLbl.textColor = STLThemeColor.col_26652C;
        } else {
            _isNewLbl.font = [UIFont boldSystemFontOfSize:12];
            _isNewLbl.backgroundColor = STLThemeColor.col_60CD8E;
