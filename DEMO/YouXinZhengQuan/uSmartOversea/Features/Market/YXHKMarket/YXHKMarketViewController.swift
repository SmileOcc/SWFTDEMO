//
//  YXHKMarketViewController.swift
//  uSmartOversea
//
...
class YXHKMarketViewController: YXHKViewController, ViewModelBased {
    
    var refreshFooter: YXRefreshAutoNormalFooter?
    
    var viewModel: YXHKMarketViewModel!
    
    lazy var refreshHeader: YXRefreshHeader = {
        YXWhiteRefreshHeader()
    }()
    
    lazy var hkLayout: YXHKMarketFlowLayout = {
        YXHKMarketFlowLayout.init()
    }()
...
//        }
    ...
    func goToStockList(title: String, code: String, sortType: YXStockRankSortType = .roc, sortDirection: Int = 1) {
        var dic: [String : Any] = [:]
        dic["title"] = title
        dic["market"] = YXMarketType.HK.rawValue
        dic["code"] = code
        ...
        if code == YXMarketSectionType.allHKStock.rankCode {
        ...
        }else if code == YXMarketSectionType.gem.rankCode {
            trackViewClickEvent(name: "GEM_Tab")
        }
    }
...
extension YXHKMarketViewController {
...
//MARK:数据源
extension YXHKMarketViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        case .industry:
            return 1
        case .stare:
            if (YXUserManager.shared().getLevel(with: kYXMarketHK) == .bmp) {
               return 1
            }
        ...
        case .gem:
            return self.viewModel.gemRank?.list?.count ?? 0
        default:
            if let items = viewModel.dataSource[section]["list"] as? Array<Any> {
                return (items as Array<Any>).count
            }
    ...
    func commonHeaderView(collectionView: UICollectionView, indexPath: IndexPath, sectionType: YXMarketSectionType) -> UICollectionReusableView {
        ...
        case .industry:
            headerView.action = { [weak self] in
                ...
                let context: [String : Any] = ["market": self.viewModel.market, "rankCode": self.viewModel.industryRankCode, "showSearch":false]
        ...
        case .gem:
            let title = YXMarketSectionType.gem.sectionName
            headerView.action = { [weak self] in
                guard let `self` = self else { return }
                self.goToStockList(title: title, code: self.viewModel.gemRankCode)
            }
        case .dailyFunding:
            
            headerView.action = { [weak self] in
                guard let `self` = self else { return }
                let dic: [String: Any] = ["market": YXMarketType.HK, "rankType": YXRankType.dailyFunding]
    ...
    func filterHeaderView(collectionView: UICollectionView, indexPath: IndexPath, sectionType: YXMarketSectionType) -> UICollectionReusableView {
        ...
        let hkAllHeaderView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: NSStringFromClass(YXMarketFilterCell.self), for: indexPath) as! YXMarketFilterCell
        
        hkAllHeaderView.commonHeaderView.title = title
        
        hkAllHeaderView.commonHeaderView.action = { [weak self] in
            guard let `self` = self else { return }
            self.goToStockList(title: title, code: self.viewModel.hkRankCode, sortType: self.viewModel.hkRankSortType, sortDirection: self.viewModel.hkRankSortDirection)
        }
        hkAllHeaderView.tapFilterItemAction = { [weak self](direction, sortType) in
            guard let `self` = self else { return }
            self.additionalType = sortType
            self.viewModel.hkRankSortDirection = direction
            self.viewModel.hkRankSortType = sortType
            self.viewModel.marketDataSourceSingle.subscribe(onSuccess: { [weak self](result) in
                self?.collectionView.reloadData()
            }).disposed(by: self.disposeBag)
            
        }
        hkAllHeaderView.tapSortButtonAction = { [weak self](direction, sortType) in
            guard let `self` = self else { return }
            self.viewModel.hkRankSortDirection = direction
            self.viewModel.hkRankSortType = sortType
            self.viewModel.marketDataSourceSingle.subscribe(onSuccess: { [weak self](result) in
...
//            indexCell.showArrowMore = true
        ...
        case .mainboard, .gem, .dailyFunding:
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        case .index:
            ...
            indexCell.midTimeLabel.text = "12:00/13:00"
            indexCell.onClickIndexPath = { [weak self](indexPath) in
                ...
                if let market = quote.market, let symbol = quote.symbol {
                    ...
                    var pageBtnStr = ""
                    if symbol == YXMarketIndex.HSI.rawValue {
                        pageBtnStr = "HSI_Tab"
                    } else if symbol == YXMarketIndex.HSCEI.rawValue {
                        pageBtnStr = "HSCEI_Tab"
                    } else if symbol == YXMarketIndex.HSTECH.rawValue {
                        pageBtnStr = "HSTECH_Tab"
                    } else if symbol == YXMarketIndex.HSCCI.rawValue {
                        pageBtnStr = "HSCCI_Tab"
                    } else if symbol == YXMarketIndex.SPHKGEM.rawValue {
                        pageBtnStr = "SPHKGEM_Tab"
                    } else if symbol == YXMarketIndex.VHSI.rawValue {
                        pageBtnStr = "VHSI_Tab"
                    }
        ...
        case .allHKStock:
            ...
            stockCell.level = viewModel.hkRank?.level
        ...
        case .gem:
            let stockCell = cell as! YXStockCollectionViewCell
            if let list = self.viewModel.gemRank?.list {
                stockCell.info = list[indexPath.item]
            }
            stockCell.level = viewModel.mainRank?.level
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        case .entrance:
            var dic: [String: Any] = [:]
            if indexPath.item == 0 {
                dic["type"] = YXRankType.adr
                self.viewModel.navigator.push(YXModulePaths.hkADR.url, context: dic)
                trackViewClickEvent(name: "HK ADRs_Tab")
                
            }else if indexPath.item == 1 {
//                self.viewModel.navigator.push(YXModulePaths.etfrank.url, context: [:])
                let vm = YXHotETFViewModel.init(services: self.viewModel.navigator, params: ["market": kYXMarketHK])
                self.viewModel.navigator.push(vm, animated: true)
                
                trackViewClickEvent(name: "HK ETFs_Tab")
            }else if indexPath.item == 2 {
                
                let dic: [String: Any] = [
                    YXWebViewModel.kWebViewModelUrl: YXH5Urls.brokerRankUrl()
                ]
                self.viewModel.navigator.push(YXModulePaths.webView.url, context: YXWebViewModel(dictionary: dic))
                trackViewClickEvent(name: "Broker Ranking_Tab")
                ...
                self.viewModel.navigator.push(vc, animated: true)
        ...
        case .allHKStock, .dailyFunding, .monthlyPayment:
            let items = list as! Array<YXMarketRankCodeListInfo>
        ...
        case .gem:
            var items = self.viewModel.gemRank?.list ?? []
...
//MARK:布局
extension YXHKMarketViewController: UICollectionViewDelegateFlowLayout {
...
//            }
        ...
        case .allHKStock, .mainboard, .gem, .dailyFunding, .monthlyPayment:
...
class YXHKMarketFlowLayout: UICollectionViewFlowLayout {
