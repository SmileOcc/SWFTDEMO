...
class YXHKHoldListCell: QMUITableViewCell, HasDisposeBag {

    lazy var stock = BehaviorRelay<YXHoldStock?>(value: nil)
    ...
    func refreshUI(model: YXHoldStock?) {

        nameLabel.text = model?.stockName ?? "--"
        symbolLabel.text = model?.stockCode ?? "--"
        if let exchangeType = model?.exchangeType {
            symbolLabel.text = symbolLabel.text! + "." + exchangeType.market.uppercased()
        }
    
        if let quoteType = model?.quoteType, quoteType == "0" {
            isDelay = true
        } else {
            isDelay = false
        }
    ...
    func setDetailLabelText(type: YXStockRankSortType, label: UILabel, model: YXHoldStock?) {
        let moneyFormatter = NumberFormatter()
        moneyFormatter.positiveFormat = "###,##0.00"
        moneyFormatter.locale = Locale(identifier: "zh")
        switch type {
        case .dailyBalance: //上市日期
            if let dailyBalancePercent = model?.dailyBalancePercent, let value = Double(dailyBalancePercent) {
                if value == 0 {
                    label.text = "0.00%"
                } else {
        ...
        case .holdingBalance:
            if let holdingBalancePercent = model?.holdingBalancePercent, let value = Double(holdingBalancePercent) {
                if value == 0 {
                    label.text = "0.00%"
                } else {
                    label.text = String(format: "%+.2f%%", value * 100)
        ...
        case .marketValueAndNumber:
            if let currentAmount = model?.currentAmount, let value = Double(currentAmount) {
                if value == 0 {
                    label.text = "0"
                } else {
                    ...
                    label.text = String(format: "%@", formatter.string(from: NSNumber(value: value)) ?? "0")
                }
            }
    ...
    func setLabelText(type: YXStockRankSortType, label: UILabel, model:YXHoldStock?) {
        switch type {
        case .dailyBalance: //上市日期
            if let dailyBalance = model?.dailyBalance, let value = Double(dailyBalance) {
                if value == 0 {
                    label.text = "0.00"
                } else {
                    let formatter = NumberFormatter()
        ...
        case .lastAndCostPrice:
            if let lastPrice = model?.lastPrice, let value = Double(lastPrice) {
                if value == 0 {
                    label.text = "0.00"
                } else {
                    label.text = String(format: "%.3f", value)
