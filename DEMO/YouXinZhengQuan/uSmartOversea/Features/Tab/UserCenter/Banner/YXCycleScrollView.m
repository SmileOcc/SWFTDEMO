...
NSString * const ID = @"YXCycleScrollViewCell";

@interface YXCycleScrollView () <UICollectionViewDataSource, UICollectionViewDelegate>
...
{
    YXCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
...
- (void)setupMainView
{
    ...
    [mainView registerClass:[YXCycleViewCell class] forCellWithReuseIdentifier:ID];
...
- (void)setPlaceholderImage:(UIImage *)placeholderImage
{
    ...
    if (!self.backgroundImageView) {
        ...
        self.backgroundColor = QMUITheme.blockColor;
    }
...
- (void)setCurrentPageDotImage:(UIImage *)currentPageDotImage
{
    ...
    if (self.pageControlStyle != YXCycleScrollViewPageContolStyleAnimated) {
        self.pageControlStyle = YXCycleScrollViewPageContolStyleAnimated;
    }
...
- (void)setTitlesGroup:(NSArray *)titlesGroup
{
    ...
    if (self.onlyDisplayText && !self.onlyDisplayTextWithImage) {
        NSMutableArray *temp = [NSMutableArray new];
...
- (void)setLabelsGroup:(NSArray *)labelsGroup{
...
- (void)layoutSubviews
{
    ...
    if (_mainView.contentOffset.x == 0 &&  _totalItemsCount) {
        if (_flowLayout.itemSize.width == self.frame.size.width && _flowLayout.itemSize.height == self.frame.size.height && (self.showPageControl == false || self.pageControlStyle == YXCycleScrollViewPageContolStyleNone || self.onlyDisplayText)) {
    ...
    CGFloat x = self.sd_width/2 - size.width/2;
    CGFloat y = self.mainView.sd_height - size.height - 5;
...
{
    YXCycleViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID forIndexPath:indexPath];
    ...
    cell.showBackView = self.showBackView;

    NSString *imagePath = self.imagePathsGroup[itemIndex];
    
    if ((!self.onlyDisplayText || (self.onlyDisplayText && self.onlyDisplayTextWithImage)) && [imagePath isKindOfClass:[NSString class]]) {
    ...
    } else if ((!self.onlyDisplayText || (self.onlyDisplayText && self.onlyDisplayTextWithImage)) && [imagePath isKindOfClass:[UIImage class]]) {
        cell.imageView.image = (UIImage *)imagePath;
    }
    
    if (_titlesGroup.count && itemIndex < _titlesGroup.count) {
        
        if (_attributeTitlesGroup.count && itemIndex < _attributeTitlesGroup.count) {
            cell.attributeTitle = _attributeTitlesGroup[itemIndex];
        } else {
    ...
    if (_labelsGroup.count && itemIndex < _labelsGroup.count) {
        NSString *label = _labelsGroup[itemIndex];
        cell.lable = label;
    ...
    if (!cell.hasConfigured) {
        ...
        cell.lineSpacing = self.lineSpacing;
        cell.onlyDisplayText = self.onlyDisplayText;
        cell.onlyDisplayTextWithImage = self.onlyDisplayTextWithImage;
    }
    cell.titleLabelTextAlignment = self.titleLabelTextAlignment;
    cell.titleLabelTextFont = self.titleLabelTextFont;
    cell.titleLabelTextColor = self.titleLabelTextColor;
...
- (void)collectionView:(UICollectionView *)collectionView
    ...
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:willDisplayCell:)]) {
        [self.delegate cycleScrollView:self willDisplayCell:cell];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView
  didEndDisplayingCell:(UICollectionViewCell *)cell
    forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:didEndDisplayingCell:)]) {
        [self.delegate cycleScrollView:self didEndDisplayingCell:cell];
    }
}
...
- (void)makeScrollViewScrollToIndex:(NSInteger)index{
    ...
    if (0 == _totalItemsCount) return;
     ...
     [_mainView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:(int)(_totalItemsCount * 0.5 + index) inSection:0] atScrollPosition:UICollectionViewScrollPositionNone animated:NO];
