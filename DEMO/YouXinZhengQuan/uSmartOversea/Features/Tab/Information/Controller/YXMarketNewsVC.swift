...
class YXMarketNewsVC: YXHKTableViewController, HUDViewModelBased {

    typealias ViewModelType = YXMarketNewsViewModel
    var viewModel: YXMarketNewsViewModel!
...
//        queryBannerInfo()
    ...
    func initUI() {
        ...
        let image = UIImage.qmui_image(with: UIColor.qmui_color(withHexString: "#FFFFFF")?.withAlphaComponent(0.3), size: CGSize.init(width: 8, height: 2), cornerRadius: 0)
        self.bannerView.pageDotImage = image
        let curImage = UIImage.qmui_image(with: UIColor.qmui_color(withHexString: "#EEEEEE"), size: CGSize.init(width: 8, height: 2), cornerRadius: 0)
...
//        self.viewModel.services.infomationService.request(.recommend(self.viewModel.cmd, self.viewModel.isTop, self.viewModel.lastTime, lanType, self.viewModel.market_str), response: self.viewModel.recommendResponse).disposed(by: self.disposeBag)
        ...
        self.viewModel.services.infomationService.request(.recommendnewsbylang(self.viewModel.isTop, self.viewModel.last_score, 30, self.viewModel.lang, self.viewModel.market_str), response: self.viewModel.recommendResponse).disposed(by: self.disposeBag)
...
extension YXMarketNewsVC: YXCycleScrollViewDelegate {
