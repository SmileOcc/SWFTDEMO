        ...
        didSet {
            if let item = model {
                ...
                if let discuss = item.discuss {
                    ...
                    let attributeString = NSMutableAttributedString.init(string: discuss, attributes: [NSAttributedString.Key.font : UIFont.normalFont14(), NSAttributedString.Key.foregroundColor :  QMUITheme().textColorLevel1(), NSAttributedString.Key.paragraphStyle : paragraph])
...
//        label.text = YXLanguageUtility.kLang(key: "reply")
    ...
    lazy var moreButton: QMUIButton = {
        let btn = QMUIButton()
        btn.setImage(UIImage(named: "theme_ygc_more"), for: .normal)
        btn.qmui_tapBlock = { [weak self] _ in
            guard let `self` = self else { return }
            self.longPressCallback?(self.model)
        }
        return btn
    }()
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        authorTag.snp.makeConstraints { make in
            ...
            make.top.equalTo(photoImageView)
            make.right.lessThanOrEqualToSuperview().offset(-46)
        }
        ...
        moreButton.qmui_tapBlock = {[weak self] _ in
            guard let `self` = self else { return }
            self.longPressCallback?(self.model)
        }
