//
//  YXThirdPhoneActivateViewController.swift
//  uSmartOversea
//
//  Created by Mac on 2019/7/18.
//  Copyright © 2019 RenRenDai. All rights reserved.
//
/* 第三方登录，绑定手机号，手机号已经预注册未激活，要输入证件号码的页面 */
import UIKit
import Reusable
import RxSwift
import RxCocoa
...
class YXThirdPhoneActivateViewController: YXHKViewController, HUDViewModelBased {
    
    var networkingHUD: YXProgressHUD! = YXProgressHUD()
    
    var viewModel: YXThirdPhoneActivateViewModel!
    ...
    func bindViewModel() {
        ...
        loginBtn.rx.tap.subscribe(onNext: {[weak self] _ in
            ...
            let areaCode = strongSelf.viewModel.areaCode
            ...
            let thirdLoginType = strongSelf.viewModel.thirdLoginType
            let accessToken = strongSelf.viewModel.accessToken
            let identifyCode = strongSelf.viewModel.idNumber.value
            let openId = strongSelf.viewModel.openId
            
            /* 三方登录预注册手机号码激活聚合接口
             three-login-guest-activate-aggregation/v1  */
            strongSelf.viewModel.services.aggregationService.request(.thirdLoginActivateAggreV1(areaCode, captcha, phone, thirdLoginType, accessToken, identifyCode, openId), response: strongSelf.viewModel.thirdActivateLoginResponse).disposed(by:strongSelf.disposeBag)
        }).disposed(by: disposeBag)
    }
...
extension YXThirdPhoneActivateViewController {
    ...
    func showLockedAlert(withMsg msg: String) {
        ...
        alertView.addAction(YXAlertAction(title: YXLanguageUtility.kLang(key: "login_backPwd"), style: .default, handler: {[weak alertView, weak self] action in
            ...
            let context = YXNavigatable(viewModel: YXForgetPwdPhoneViewModel(with: self.viewModel.areaCode, phone: self.viewModel.phone.value, isLogin: false, callBack: self.fillPhone, loginCallBack: loginCallBack, fromVC:self,sourceVC:self.viewModel.vc))
