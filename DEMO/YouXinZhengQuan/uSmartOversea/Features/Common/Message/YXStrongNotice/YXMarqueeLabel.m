...
#import "YXMarqueeLabel.h"

@interface YXMarqueeLabel ()
...
/// 该参数记录原本设置的 lineBreakMode，当关闭走马灯动画后恢复 lineBreakMode 为该值
@property(nonatomic, assign) NSLineBreakMode originalLineBreakMode;

/// 该参数记录原本设置的 numberOfLines，当关闭走马灯动画后恢复 numberOfLines 为该值
@property(nonatomic, assign) NSLineBreakMode originalNumberOfLines;

@end

@implementation YXMarqueeLabel
...
- (void)didInitialize {
    ...
    self.enableMarqueeAnimation = NO;
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    if (![self.attributedText isEqualToAttributedString:self.attributedText]) {
        self.offsetX = 0;
    }
...
- (void)setEnableMarqueeAnimation:(BOOL)enableMarqueeAnimation {
    _enableMarqueeAnimation = enableMarqueeAnimation;
    self.numberOfLines = self.originalNumberOfLines;
    self.lineBreakMode = self.originalLineBreakMode;
    [self setNeedsLayout];
}

- (void)drawTextInRect:(CGRect)rect {
    if (!self.enableMarqueeAnimation) {
...
- (BOOL)shouldPlayDisplayLink {
    BOOL result = self.enableMarqueeAnimation && self.window && CGRectGetWidth(self.bounds) > 0 && self.textSize.width > CGRectGetWidth(self.bounds);
...
- (void)setNumberOfLines:(NSInteger)numberOfLines {
    _originalNumberOfLines = numberOfLines;
    if (self.enableMarqueeAnimation) { // 当开启走马灯动画时，需要强制设置 numberOfLines 为 1
        numberOfLines = 1;
    }
...
- (void)setLineBreakMode:(NSLineBreakMode)lineBreakMode {
    _originalLineBreakMode = lineBreakMode;
    if (self.enableMarqueeAnimation) { // 开启走马灯动画时，需要强制设置 lineBreakMode 为 NSLineBreakByClipping
...
@implementation YXMarqueeLabel (ReusableView)
