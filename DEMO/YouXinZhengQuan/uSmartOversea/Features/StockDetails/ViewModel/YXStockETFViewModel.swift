...
class YXStockETFViewModel: HUDServicesViewModel, RefreshViewModel {
    ...
    //响应回调
    var etfResultResponse: YXResultResponse<YXMarketRankModel>?
    
    var sorttype: YXStockRankSortType  = .now
    ...
    lazy var userLevel: QuoteLevel = {
        ...
        YXUserManager.shared().getLevel(with: YXMarketType.US.rawValue)
    }()

    func updateLevel() {
        userLevel = YXUserManager.shared().getLevel(with: YXMarketType.US.rawValue)
    }
    ...
    var dataSourceSingle: Single<[IdentifiableModel]> {
        Single<[YXMarketRankCodeListInfo]>.create(subscribe: { [weak self] (single) -> Disposable in
            ...
            return strongSelf.services.request(.quotesRank(sortType: strongSelf.sorttype.rawValue, sortDirection: strongSelf.sortdirection, pageDirection: 0, from: strongSelf.offsetForPage(strongSelf.page), count: strongSelf.perPage, code: kYXIndexIXICETF, market: YXMarketType.US.rawValue, level: strongSelf.userLevel.rawValue), response: { (response: YXResponseType<YXMarketRankModel>) in
    ...
    init() {
        ...
        etfResultResponse = { [weak self] (response) in
