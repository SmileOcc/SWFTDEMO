...
private struct ListViewAssociatedKey {
    static var endRefreshCallBack: String = "endRefreshCallBack"
}
...
extension JXPagingViewListViewDelegate where Self: UIViewController {
    ...
    var endRefreshCallBack: (() -> Void)? {
        set {
            objc_setAssociatedObject(self, &ListViewAssociatedKey.endRefreshCallBack, newValue, .OBJC_ASSOCIATION_COPY_NONATOMIC)
        }
        get {
            return (objc_getAssociatedObject(self, &ListViewAssociatedKey.endRefreshCallBack) as? (() -> Void))
        }
    }
}
