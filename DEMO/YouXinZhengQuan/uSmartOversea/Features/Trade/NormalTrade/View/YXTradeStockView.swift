...
class YXTradeStockView: UIView, YXTradeHeaderSubViewProtocol {
    
    typealias ClickBlock = () -> Void
    @objc var clickBlock: ClickBlock?
    @objc var refreshBlock: ClickBlock?
    @objc var optionKeyBlock: ClickBlock?
    ...
    lazy var textField: YXTextField = {
        ...
        textFld.font = .systemFont(ofSize: 18, weight: .medium)
        textFld.adjustsFontSizeToFitWidth = true
        textFld.minimumFontSize = 10
        textFld.attributedPlaceholder = NSAttributedString(string: YXLanguageUtility.kLang(key: "trade_search"), attributes: [.foregroundColor: QMUITheme().textColorLevel4(), .font: UIFont.systemFont(ofSize: 16, weight: .medium)])
...
//        label.contentEdgeInsets = UIEdgeInsets.init(top: , left: 0, bottom: 1, right: 0)
    ...
    @objc func optionClick(_ sender: UIButton) {
        optionKeyBlock?()
    }
}

extension YXTradeStockView : CAAnimationDelegate{
