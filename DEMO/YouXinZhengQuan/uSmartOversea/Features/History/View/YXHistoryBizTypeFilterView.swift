//
//  YXHistoryBizTypeFilterView.swift
//  uSmartOversea
//
...
class YXHistoryBizTypeFilterView: UIControl {

    let bag = DisposeBag()

    var contentViewHeight: CGFloat = 235

    var selectedButton: YXHistoryBizTypeButton!

    var filter:((_ type: YXHistoryBizType) -> Void)?

    lazy var titleLabel: UILabel = {
        let label = UILabel()
        label.text = YXLanguageUtility.kLang(key: "common_types_title")
    ...
    func show(selectedType: YXHistoryBizType) {
        self.isHidden = false
        for filterButton in filterButtonView.subviews {
            if let button = filterButton as? YXHistoryBizTypeButton {
                if button.type == selectedType {
    ...
    override init(frame: CGRect) {
        ...
        let filters: [YXHistoryBizType] = YXHistoryBizType.allCases
        ...
        for (index, filterType) in filters.enumerated() {
            let button = YXHistoryBizTypeButton.init(style: .white)
    ...
    @objc func tapFilterButton(button: YXHistoryBizTypeButton) {
    ...
    @objc func clickConfirmButton() {
        hidden()
        filter?(selectedButton.type)
    }
}

class YXHistoryBizTypeButton: YXDateFilterButton {
    var type: YXHistoryBizType!
