...
+ (NSString *)dateStringWithTimeIntervalSince1970:(NSInteger)interval{
    ...
    if (timeOff < 3600 && timeOff>=0) {
        ...
        return [NSString stringWithFormat:@"%ld分钟前", (NSInteger)minute];
...
+ (NSString*)strMMdd:(NSDate*)date
{
    return [self formatDateWithString:@"MM.dd" date:date];
}
...
+ (NSDate*)dateWithStr:(NSString*)str dateFormat:(NSString*)format timeZone:(NSTimeZone*)timeZone
{
    NSDateFormatter* formatter = [NSDateFormatter en_US_POSIXFormatter];
    //[NSTimeZone defaultTimeZone];
    //NSTimeZone* timeZone = [NSTimeZone timeZoneWithName:name]; //"Asia/Shanghai"
