...
- (void)setUpPageControl
{
    ...
    CGFloat y = self.xl_height - size.height - 20;
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        TAPageControl *pageControl = (TAPageControl *)_pageControl;
...
- (void)setUpToolBars
{
    ...
    [saveButton setTitle:[YXLanguageUtility kLangWithKey:@"common_no_download"] forState:UIControlStateNormal];
    [saveButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    saveButton.backgroundColor = [UIColor colorWithRed:0.1f green:0.1f blue:0.1f alpha:0.90f];
    saveButton.layer.cornerRadius = 5;
    saveButton.clipsToBounds = YES;
    [saveButton addTarget:self action:@selector(saveImage) forControlEvents:UIControlEventTouchUpInside];
    CGSize saveSize = [saveButton sizeThatFits:CGSizeMake(100, 25)];
    saveButton.frame = CGRectMake(self.bounds.size.width - saveSize.width - 20, self.bounds.size.height - saveSize.height - 40, saveSize.width, saveSize.height);
...
- (UIWindow *)findTheMainWindow
{
    ...
    for (UIWindow *window in frontToBackWindows) {
        ...
        if(windowOnMainScreen && windowIsVisible && windowLevelSupported && windowSizeIsEqualToScreen && ![window isKindOfClass:NSClassFromString(@"YYTextEffectWindow")]) {
            return window;
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    pageNum = (pageNum == self.imageCount) ? pageNum - 1 : pageNum ;
    if (pageNum < 0) {
        pageNum = 0;
    }
    if (pageNum >= (self.imageCount - 1)) {
        pageNum = (self.imageCount - 1);
    }
    self.currentImageIndex = pageNum;
...
/**
 一行代码展示 带保存按钮 (在某些使用场景,不需要做很复杂的操作,例如不需要长按弹出actionSheet,从而不需要实现数据源方法和代理方法,那么可以选择这个方法,直接传数据源数组进来,框架内部做处理)
 ...
 @return XLPhotoBrowser实例对象
 */
+ (instancetype)showPhotoBrowserAndSaveWithImages:(NSArray *)images currentImageIndex:(NSInteger)currentImageIndex {
    if (images.count <=0 || images ==nil) {
        XLPBLog(@"一行代码展示图片浏览的方法,传入的数据源为空,不进入图片浏览,请检查传入数据源");
        return nil;
    }
