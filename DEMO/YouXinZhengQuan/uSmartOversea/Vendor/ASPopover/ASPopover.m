...
//  _option.offset = 0;
  _option.arrowSize = CGSizeMake(0, 0);
  if (_option.autoAjustDirection) {
    ...
    BOOL canBeUp = (upPoint.y - _option.arrowSize.height - contentView.bounds.size.height > YXConstant.navBarHeight);

    CGFloat tabBarHeight = YXConstant.tabBarHeight;
    if ([UIDevice currentDevice].orientation == UIDeviceOrientationLandscapeLeft || [UIDevice currentDevice].orientation == UIDeviceOrientationLandscapeRight) {
        tabBarHeight = 0;
    }
    BOOL canBeDown = downPoint.y + _option.arrowSize.height + contentView.bounds.size.height < inView.bounds.size.height - tabBarHeight;
    ...
    if (canBeUp && !canBeDown) {
      ...
      _option.offset = upPoint.y - _option.arrowSize.height - contentView.bounds.size.height - YXConstant.navBarHeight;
      _option.arrowSize = CGSizeMake(0, 0);
      _option.popoverType = _option.preferedType;
    }
...
- (void)show:(UIView *)contentView atPoint:(CGPoint)point inView:(UIView *)inView {
  ...
  self.contentView.layer.backgroundColor = QMUITheme.popupLayerColor.CGColor;
  self.contentView.layer.shadowColor = [UIColor themeColorWithNormal:[QMUITheme.textColorLevel1 colorWithAlphaComponent:0.2] andDarkColor:UIColor.clearColor].CGColor;
  self.contentView.layer.shadowOffset = CGSizeMake(0, 2);
  self.contentView.layer.shadowOpacity = 1;
  self.contentView.layer.shadowRadius = _option.cornerRadius;
...
- (CGPoint)arrowPointWithView:(UIView *)contentView fromView:(UIView *)fromView inView:(UIView *)inView popoverType:(ASPopoverType)type {
  ...
  switch (type) {
    case ASPopoverTypeUp: {
      ...
      point = CGPointMake(point.x, point.y - _option.offset);
    } break;
...
- (void)create {
  ...
  if (outerSideEdge >= 0) {
    frame.origin.x -= (outerSideEdge + sideEdge);
      ...
      CGFloat remainSideEdge = self.containerView.bounds.size.width - CGRectGetWidth(frame);
      if (CGRectGetMinX(frame) < 0) {
          frame.origin.x += fabs(CGRectGetMinX(frame)) + sideEdge;
      } else if (CGRectGetMinX(frame) < sideEdge && remainSideEdge >= 2 * sideEdge) {
          frame.origin.x = sideEdge;
      } else if (remainSideEdge >= 2 * sideEdge && fabs(outerSideEdge) < sideEdge) {
          frame.origin.x = self.containerView.bounds.size.width - CGRectGetWidth(frame) - sideEdge;
      }
  }
  self.frame = frame;
