...
public class YXPassportReaderTip: NSObject {
    public var readyTip: String = ""
    public var readingTip: String = ""
    public var errorTip: String = ""
    public var oneMoreTagsTip: String = ""
    public var notValidTip: String = ""
    public var connectErrorTip: String = ""
    public var authenticatingTip: String = ""
...
public class PassportReader : NSObject {
    ...
    private var tip: YXPassportReaderTip!
    ...
    public func readPassport( mrzKey : String,tags: [DataGroupId], tip:YXPassportReaderTip, completed: @escaping (NFCPassportModel?, TagError?)->() ) {
        ...
        if NFCTagReaderSession.readingAvailable {
            ...
            readerSession?.alertMessage = self.tip.readyTip
            readerSession?.begin()
        }
    }
}
...
extension PassportReader : NFCTagReaderSessionDelegate {
    ...
    public func tagReaderSession(_ session: NFCTagReaderSession, didDetect tags: [NFCTag]) {
        ...
        if tags.count > 1 {
            session.alertMessage = self.tip.oneMoreTagsTip
            return
        }
        ...
        default:
            ...
            session.invalidate(errorMessage: self.tip.notValidTip)
            self.scanCompletedHandler( nil, TagError.PassportInvalid)
            return
        }
        ...
        session.connect(to: tag) { [unowned self] (error: Error?) in
            if error != nil {
                session.invalidate(errorMessage: self.tip.connectErrorTip)
                self.scanCompletedHandler( nil, TagError.ConnectionFailed)
                return
            }
    ...
    func startReading() {
        self.handleBAC(completed: { [weak self] error in
            ...
            if error == nil {
                ...
                // session keys
                self.readerSession?.alertMessage = self.tip.readingTip//"Reading passport data....."
                
                self.readNextDataGroup( ) { [weak self] error in
                    guard let `self` = self else { return }
                    if error != nil {
                        self.readerSession?.invalidate(errorMessage: self.tip.errorTip )
