...
public class YXMoyaConfig {
    ...
    private final class func certificateFile() -> URL? {
        ...
        let file = document.appendingPathComponent(YXMoyaConfig.certFileName)
        return file
    }
    
    fileprivate final class func appHttpsEvaluatorsKey() -> String {
        return YXUrlRouterConstant.appHttpsEvaluatorsKey()
    }
    ...
    public final class func interceptor() -> RequestInterceptor? {
        return Retrier { (request, session, error, completion) in
            if let urlString: String = request.request?.url?.absoluteString {
                if urlString.hasSuffix(YXRetryConfigAPI.guests.path) {
    ...
    public final class func session() -> Session {
        ...
        return Session(
            ...
            interceptor : YXMoyaConfig.interceptor(),
            serverTrustManager : YXMoyaConfig.serverTrustManager()
        )
    }
}

public class YXServerTrustManager: ServerTrustManager {
