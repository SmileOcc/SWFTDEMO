//
//  YXNetworkLoggerPlugin.swift
//  YouXinZhengQuan
//
//  Created by 胡华翔 on 2019/7/29.
...
public final class YXNetworkLoggerPlugin: PluginType {
    ...
    public init(verbose: Bool = false, cURL: Bool = false, output: ((_ separator: String, _ terminator: String, _ success: Bool, _ items: Any...) -> Void)? = nil, requestDataFormatter: ((Data) -> (String))? = nil, responseDataFormatter: ((Data) -> (Data))? = nil) {
        ...
        self.output = output ?? YXNetworkLoggerPlugin.reversedPrint
...
private extension YXNetworkLoggerPlugin {
    ...
    func logNetworkResponse(_ response: HTTPURLResponse?, data: Data?, target: TargetType) -> [String] {
        defer {
            if response != nil
                ...
                YXRealLogger.shareInstance.realLog(type: "ApiError", name: "接口Error", url: response?.url?.absoluteString, code: "\(response?.statusCode ?? 0)", desc: "", extend_msg: "")
            }
        }
...
fileprivate extension YXNetworkLoggerPlugin {
