...
class YXShortVideoItemSGView: UIView,YXShortVideoItemViewProtocol {
    ...
    lazy var slider: YXProgressSliderBar = {
        ...
        s.slider.qmui_thumbColor = QMUITheme().mainThemeColor()
        s.slider.tintColor = QMUITheme().mainThemeColor()
        s.slider.maximumTrackTintColor = .white.withAlphaComponent(0.3)
        s.slider.minimumTrackTintColor = QMUITheme().mainThemeColor()
        return s
    }()
    ...
    lazy var lessonDescLabel: YXFoldLabel = {
        ...
        label.label.textColor = UIColor(hexString:"#F5F5F5")
        return label
    }()
    
    lazy var learnNowBtn: QMUIButton = {
        let learnNow = QMUIButton()
        learnNow.backgroundColor = QMUITheme().mainThemeColor()
        learnNow.cornerRadius = 4
    ...
    lazy var animateView: LOTAnimationView = {
        let animateView = LOTAnimationView(name: YXThemeTool.isDarkMode() ? "like_animation_dark" : "like_animation_white")
    ...
    lazy var foldButton:  QMUIButton = {
        ...
        button.setTitle(YXLanguageUtility.kLang(key: "beerich_comment_btn_more"), for: .normal)
        button.setTitle(YXLanguageUtility.kLang(key: "beerich_comment_btn_less"), for: .selected)
    ...
    lazy var bottomSheet: YXBottomSheetViewTool = {
        ...
        sheet.header.backgroundColor = QMUITheme().foregroundColor()
...
//        }
        ...
        foldButton.snp.makeConstraints { make in
            make.top.equalTo(lessonDescLabel.snp.bottom).offset(8)
    ...
    func updateUI(isFullScreen: Bool) {
        ...
        var rightSideOffset = isFullScreen ? -YXConstant.safeAreaInsetsBottomHeight()-77 : -72
        if courseDetailView.isHidden {
            rightSideOffset += 40
        }
        
        rightSideView.snp.remakeConstraints { make in
            make.width.equalTo(44)
            make.right.equalToSuperview().offset(-10)
            make.bottom.equalToSuperview().offset(rightSideOffset)
        }
        
        courseDetailView.snp.remakeConstraints { make in
            make.bottom.equalToSuperview().offset(isFullScreen ? -YXConstant.safeAreaInsetsBottomHeight()-15 : 0)
            make.left.right.equalToSuperview()
            make.height.equalTo(40)
        }
        
        slider.isHidden = !isFullScreen
