...
//#import "HCRefreshAuotNormalFooter.h"
#import "STLBlankPageTipView.h"
#import "URLDEMO-Swift.h"

/** 网络连接失败 */
//#define kNetworkConnectDefaultFailTips STLLocalizedString_(@"BlankPage_NetworkError_tipTitle",nil)
#define kNetworkConnectDefaultFailTips @"EmptyCustomViewManager_titleLabel"
/** 刷新提示文案 */
#define kAgainRequestDefaultTipString  @"EmptyCustomViewManager_refreshButton"
/** 请求失败默认提示文案 */
#define kReqFailDefaultTipText         @"EmptyCustomViewManager_titleLabel"
/** 请求空数据默认提示文案 */
#define kEmptyDataDefaultTipText       @"EmptyCustomViewHasNoData_titleLabel"
/* 弱引用 */
...
- (void)showRequestTip:(NSDictionary *)totalPageCurrentPageCountDict
{
    ...
    if ([self contentViewIsEmptyData]) {//页面没有数据
        ...
        //根据状态,显示背景提示Viwe
        if (![HCNetworkReachabilityManager shared].isReachable) {
            //显示没有网络提示
...
//            //成功:显示空数据提示, 失败:显示请求失败提示
        ...
        if (!requestSuccess && self.mj_header) {
            ...
            if (!vcView || vcView.bounds.size.height < 600.0) {
                vcView = nil;
            }
...
- (void)showRequestTip:(NSDictionary *)totalPageCurrentPageCountDict isNeedNetWorkStatus:(BOOL)isNeedNetWorkStatus
{
    ...
    if ([self contentViewIsEmptyData]) {//页面没有数据
        ...
        //根据状态,显示背景提示Viwe
        if (![HCNetworkReachabilityManager shared].isReachable && isNeedNetWorkStatus) {
            //显示没有网络提示
            [self showTipWithStatus:RequesNoNetWorkStatus];
